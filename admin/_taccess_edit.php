...
$layout = new TLayout("edit2","BoldAqua","MobileAqua");
...
$layout->blocks["top"][] = "details";$page_layouts["_taccess_edit"] = $layout;
...
$templatefile = ($inlineedit == EDIT_INLINE) ? "_taccess_inline_edit.htm" : "_taccess_edit.htm";
...
//	processing Zip - begin
	$condition = 1;

	if($condition)
	{
		$control_Zip = $pageObject->getControl("Zip", $id);
		$control_Zip->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);
...
//	processing Location - begin
	...
	if($condition)
	{
		...
		$control_Location->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing Location - end
//	processing Start - begin
	$condition = 1;

	if($condition)
	{
		$control_Start = $pageObject->getControl("Start", $id);
		$control_Start->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing Start - end
//	processing End - begin
	$condition = 1;

	if($condition)
	{
		$control_End = $pageObject->getControl("End", $id);
		$control_End->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);
...
//	processing Note - end
	...
	//	processing Country - end
			//	processing Zip - begin
				...
				if($condition)
				{
					$control_Zip->afterSuccessfulSave();
				}
	//	processing Zip - end
			//	processing Location - begin
							$condition = 1;
	...
	//	processing Location - end
			//	processing Start - begin
				...
				if($condition)
				{
					$control_Start->afterSuccessfulSave();
				}
	//	processing Start - end
			//	processing End - begin
							$condition = 1;
				...
				if($condition)
				{
					$control_End->afterSuccessfulSave();
				}
	//	processing End - end
			//	processing Note - begin
							$condition = 1;
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// redirect
	header("Location: _taccess_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: _taccess_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($inlineedit != EDIT_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("Zip_label","<label for=\"".GetInputElementId("Zip", $id, PAGE_EDIT)."\">","</label>");
	...
	if(!$pageObject->isAppearOnTabs("Start"))
		$xt->assign("Start_fieldblock",true);
	else
		$xt->assign("Start_tabfieldblock",true);
	$xt->assign("Start_label",true);
	if(isEnableSection508())
		$xt->assign_section("Start_label","<label for=\"".GetInputElementId("Start", $id, PAGE_EDIT)."\">","</label>");
		
	if(!$pageObject->isAppearOnTabs("End"))
		$xt->assign("End_fieldblock",true);
	else
		$xt->assign("End_tabfieldblock",true);
	$xt->assign("End_label",true);
	if(isEnableSection508())
		$xt->assign_section("End_label","<label for=\"".GetInputElementId("End", $id, PAGE_EDIT)."\">","</label>");
...
//	Zip - 
	...
	$showValues["Zip"] = $value;
	$showFields[] = "Zip";
		$showRawValues["Zip"] = substr($data["Zip"],0,100);
...
//	Start - Short Date
	$value = $pageObject->showDBValue("Start", $data, $keylink);
	$showValues["Start"] = $value;
	$showFields[] = "Start";
		$showRawValues["Start"] = substr($data["Start"],0,100);

//	End - Short Date
	$value = $pageObject->showDBValue("End", $data, $keylink);
	$showValues["End"] = $value;
	$showFields[] = "End";
		$showRawValues["End"] = substr($data["End"],0,100);
...
foreach($pageObject->editFields as $fName)
{
	...
	if($inlineedit == EDIT_INLINE)
	{
		...
		if($pageObject->pSet->isUseRTE("Country") && $fName == "Country")
	{
		$_SESSION[$strTableName."_"."Country"."_rte"] = "Schweiz";
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='_taccess_view.php?".$viewlink."'\"");
