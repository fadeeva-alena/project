...
$layout->blocks["top"][] = "details";$page_layouts["t_needs_view"] = $layout;
...
if (!sizeof($data)) {
	header("Location: t_needs_list.php?a=return");
	exit();
}
...
while($data)
{
	$xt->assign("show_key1", htmlspecialchars($pageObject->showDBValue("need_id", $data)));
...
//need_id - 
	...
	$value = $pageObject->showDBValue("need_id", $data, $keylink);
	if($mainTableOwnerID=="need_id")
		$ownerIdValue=$value;
	$xt->assign("need_id_value",$value);
	if(!$pageObject->isAppearOnTabs("need_id"))
		$xt->assign("need_id_fieldblock",true);
	else
		$xt->assign("need_id_tabfieldblock",true);
////////////////////////////////////////////
//people_id - 
...
//need_type_id - 
	...
	$value = $pageObject->showDBValue("need_type_id", $data, $keylink);
	if($mainTableOwnerID=="need_type_id")
		$ownerIdValue=$value;
	$xt->assign("need_type_id_value",$value);
	if(!$pageObject->isAppearOnTabs("need_type_id"))
		$xt->assign("need_type_id_fieldblock",true);
	else
		$xt->assign("need_type_id_tabfieldblock",true);
////////////////////////////////////////////
//need_subtype_id - 
	
	$value = $pageObject->showDBValue("need_subtype_id", $data, $keylink);
	if($mainTableOwnerID=="need_subtype_id")
		$ownerIdValue=$value;
	$xt->assign("need_subtype_id_value",$value);
	if(!$pageObject->isAppearOnTabs("need_subtype_id"))
		$xt->assign("need_subtype_id_fieldblock",true);
	else
		$xt->assign("need_subtype_id_tabfieldblock",true);
////////////////////////////////////////////
//need_note - 
	
	$value = $pageObject->showDBValue("need_note", $data, $keylink);
	if($mainTableOwnerID=="need_note")
		$ownerIdValue=$value;
	$xt->assign("need_note_value",$value);
	if(!$pageObject->isAppearOnTabs("need_note"))
		$xt->assign("need_note_fieldblock",true);
	else
		$xt->assign("need_note_tabfieldblock",true);
////////////////////////////////////////////
//need_hourly - 
	
	$value = $pageObject->showDBValue("need_hourly", $data, $keylink);
	if($mainTableOwnerID=="need_hourly")
		$ownerIdValue=$value;
	$xt->assign("need_hourly_value",$value);
	if(!$pageObject->isAppearOnTabs("need_hourly"))
		$xt->assign("need_hourly_fieldblock",true);
	else
		$xt->assign("need_hourly_tabfieldblock",true);
////////////////////////////////////////////
//prof_provider - 
...
$xt->assign("editlink_attrs","id=\"editLink".$id."\" name=\"editLink".$id."\" onclick=\"window.location.href='t_needs_edit.php?".$editlink."'\"");
