...
function GetDefaultValue($field, $ptype, $table="")
{
	...
	if(!$table)
		$table=$strTableName;
				if($table=="_taccess" && $field=="Country") 
	{
		return "Schweiz";
	}
				if($table=="_taccess" && $field=="Start") 
	{
		return now();
	}
				if($table=="_taccess" && $field=="End") 
	{
...
function runner_mail_smtp($params)
{
	...
	if("0" == "1" && "" != "" || isset($params['host']))
	{
		$mail->Host = isset($params['host']) ? $params['host'] : "";     // sets SMTP server
	}
	else if(ini_get('SMTP') != '')
		$mail->Host = ini_get('SMTP');
		
	if("0" == "1" && "" != "" || isset($params['port']))
		$mail->Port = isset($params['port']) ? $params['port'] + 0 : ""+0;     	// set the SMTP port
