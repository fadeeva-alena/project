...
function checkTableName($shortTName, $type=false)
{
	...
	if ("_taccess" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("_site_mode" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("t_log" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("_t_search" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("t_people" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("t_skills" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("t_needs" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	return false;
}
...
function inFacebook(){
	...
	if ($signed_request) {
		...
		$strSQL = "select * from ".AddTableWrappers("t_people")." where ".AddFieldWrappers(GetUserNameField())."='".$strUsername."'";
...
function AfterFBLogIn($pUsername, $pPassword){
	...
	$strSQL = "select * from ".AddTableWrappers("t_people")." where ".AddFieldWrappers($cUserNameField)."=".$strUsername."";
	$rs = db_query($strSQL,$conn);
 	$data = db_fetch_array($rs);
	if($data){
		$logged=true;
