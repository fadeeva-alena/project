...
$layout->blocks["top"][] = "details";$page_layouts["_taccess_add"] = $layout;
...
if(isset($_REQUEST['afteradd'])){
		header('Location: _taccess_add.php');
...
//	processing Zip - start
	...
	if($inlineAddOption)
	{
		...
		$control_Zip->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing Zip - end
...
//	processing Start - start
	...
	if($inlineAddOption)
	{
		...
		$control_Start->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
...
//	processing End - start
	...
	if($inlineAddOption)
	{
		...
		$control_End->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing End - end
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'_taccess_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'_taccess_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: _taccess_".$pageObject->getPageType().".php");
...
if($inlineadd!=ADD_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("Zip_label","<label for=\"".GetInputElementId("Zip", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("Start_label","<label for=\"".GetInputElementId("Start", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("End_label","<label for=\"".GetInputElementId("End", $id, PAGE_ADD)."\">","</label>");
