...
$layout->blocks["top"][] = "search";$page_layouts["t_needs_search"] = $layout;
...
if ($mode==SEARCH_SIMPLE)
{
	...
	if($pageObject->pSet->getLookupTable("need_id"))
		$pageObject->settingsMap["globalSettings"]['shortTNames'][$pageObject->pSet->getLookupTable("need_id")] = GetTableURL($pageObject->pSet->getLookupTable("need_id"));
	
	$pageObject->fillFieldToolTips("need_id");	
	
	$srchFields = $pageObject->searchClauseObj->getSearchCtrlParams("need_id");
	...
	{
		$firstFieldParams['fName'] = "need_id";
	...
	if(isEnableSection508())
		$xt->assign_section("need_id_label","<label for=\"".GetInputElementId("need_id", $id, PAGE_SEARCH)."\">","</label>");
	else 
		$xt->assign("need_id_label", true);
	
	$xt->assign("need_id_fieldblock", true);
	$xt->assignbyref("need_id_editcontrol", $ctrlBlockArr['searchcontrol']);
	$xt->assign("need_id_notbox", $ctrlBlockArr['notbox']);
	// create second control, if need it
	$xt->assignbyref("need_id_editcontrol1", $ctrlBlockArr['searchcontrol1']);
	// create search type select
	$xt->assign("searchtype_need_id", $ctrlBlockArr['searchtype']);
	$isFieldNeedSecCtrl = $searchControlBuilder->isNeedSecondCtrl("need_id");
	$ctrlInd = 0;
	if ($isFieldNeedSecCtrl)
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"need_id", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd, 1=>($ctrlInd+1)));
		$ctrlInd+=2;
	}
	else
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"need_id", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd));			
		$ctrlInd++;
	}
	// search fields data
	...
	if($pageObject->pSet->getLookupTable("need_type_id"))
		$pageObject->settingsMap["globalSettings"]['shortTNames'][$pageObject->pSet->getLookupTable("need_type_id")] = GetTableURL($pageObject->pSet->getLookupTable("need_type_id"));
	
	$pageObject->fillFieldToolTips("need_type_id");	
	
	$srchFields = $pageObject->searchClauseObj->getSearchCtrlParams("need_type_id");
	$firstFieldParams = array();
	if (count($srchFields))
	{
		$firstFieldParams = $srchFields[0];
	...
	{
		$firstFieldParams['fName'] = "need_type_id";
		$firstFieldParams['eType'] = '';
		$firstFieldParams['value1'] = '';
		$firstFieldParams['opt'] = '';
		$firstFieldParams['value2'] = '';
	...
	if(isEnableSection508())
		$xt->assign_section("need_type_id_label","<label for=\"".GetInputElementId("need_type_id", $id, PAGE_SEARCH)."\">","</label>");
	else 
		$xt->assign("need_type_id_label", true);
	
	$xt->assign("need_type_id_fieldblock", true);
	$xt->assignbyref("need_type_id_editcontrol", $ctrlBlockArr['searchcontrol']);
	$xt->assign("need_type_id_notbox", $ctrlBlockArr['notbox']);
	// create second control, if need it
	$xt->assignbyref("need_type_id_editcontrol1", $ctrlBlockArr['searchcontrol1']);
	// create search type select
	$xt->assign("searchtype_need_type_id", $ctrlBlockArr['searchtype']);
	$isFieldNeedSecCtrl = $searchControlBuilder->isNeedSecondCtrl("need_type_id");
	$ctrlInd = 0;
	if ($isFieldNeedSecCtrl)
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"need_type_id", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd, 1=>($ctrlInd+1)));
		$ctrlInd+=2;
	}
	else
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"need_type_id", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd));			
		$ctrlInd++;
	}
	// search fields data
	
	if($pageObject->pSet->getLookupTable("need_subtype_id"))
		$pageObject->settingsMap["globalSettings"]['shortTNames'][$pageObject->pSet->getLookupTable("need_subtype_id")] = GetTableURL($pageObject->pSet->getLookupTable("need_subtype_id"));
	
	$pageObject->fillFieldToolTips("need_subtype_id");	
	
	$srchFields = $pageObject->searchClauseObj->getSearchCtrlParams("need_subtype_id");
	$firstFieldParams = array();
	if (count($srchFields))
	{
		$firstFieldParams = $srchFields[0];
	...
	{
		$firstFieldParams['fName'] = "need_subtype_id";
		$firstFieldParams['eType'] = '';
		$firstFieldParams['value1'] = '';
		$firstFieldParams['opt'] = '';
		$firstFieldParams['value2'] = '';
	...
	if(isEnableSection508())
		$xt->assign_section("need_subtype_id_label","<label for=\"".GetInputElementId("need_subtype_id", $id, PAGE_SEARCH)."\">","</label>");
	else 
		$xt->assign("need_subtype_id_label", true);
	
	$xt->assign("need_subtype_id_fieldblock", true);
	$xt->assignbyref("need_subtype_id_editcontrol", $ctrlBlockArr['searchcontrol']);
	$xt->assign("need_subtype_id_notbox", $ctrlBlockArr['notbox']);
	// create second control, if need it
	$xt->assignbyref("need_subtype_id_editcontrol1", $ctrlBlockArr['searchcontrol1']);
	// create search type select
	$xt->assign("searchtype_need_subtype_id", $ctrlBlockArr['searchtype']);
	$isFieldNeedSecCtrl = $searchControlBuilder->isNeedSecondCtrl("need_subtype_id");
	$ctrlInd = 0;
	if ($isFieldNeedSecCtrl)
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"need_subtype_id", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd, 1=>($ctrlInd+1)));
		$ctrlInd+=2;
	}
	else
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"need_subtype_id", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd));			
		$ctrlInd++;
	}
	// search fields data
	
	if($pageObject->pSet->getLookupTable("need_note"))
		$pageObject->settingsMap["globalSettings"]['shortTNames'][$pageObject->pSet->getLookupTable("need_note")] = GetTableURL($pageObject->pSet->getLookupTable("need_note"));
	
	$pageObject->fillFieldToolTips("need_note");	
	
	$srchFields = $pageObject->searchClauseObj->getSearchCtrlParams("need_note");
	$firstFieldParams = array();
	if (count($srchFields))
	{
		$firstFieldParams = $srchFields[0];
	...
	{
		$firstFieldParams['fName'] = "need_note";
		$firstFieldParams['eType'] = '';
		$firstFieldParams['value1'] = '';
		$firstFieldParams['opt'] = '';
		$firstFieldParams['value2'] = '';
	...
	if(isEnableSection508())
		$xt->assign_section("need_note_label","<label for=\"".GetInputElementId("need_note", $id, PAGE_SEARCH)."\">","</label>");
	else 
		$xt->assign("need_note_label", true);
	
	$xt->assign("need_note_fieldblock", true);
	$xt->assignbyref("need_note_editcontrol", $ctrlBlockArr['searchcontrol']);
	$xt->assign("need_note_notbox", $ctrlBlockArr['notbox']);
	// create second control, if need it
	$xt->assignbyref("need_note_editcontrol1", $ctrlBlockArr['searchcontrol1']);
	// create search type select
	$xt->assign("searchtype_need_note", $ctrlBlockArr['searchtype']);
	$isFieldNeedSecCtrl = $searchControlBuilder->isNeedSecondCtrl("need_note");
	$ctrlInd = 0;
	if ($isFieldNeedSecCtrl)
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"need_note", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd, 1=>($ctrlInd+1)));
		$ctrlInd+=2;
	}
	else
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"need_note", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd));			
		$ctrlInd++;
	}
	// search fields data
	
	if($pageObject->pSet->getLookupTable("need_hourly"))
		$pageObject->settingsMap["globalSettings"]['shortTNames'][$pageObject->pSet->getLookupTable("need_hourly")] = GetTableURL($pageObject->pSet->getLookupTable("need_hourly"));
	
	$pageObject->fillFieldToolTips("need_hourly");	
	
	$srchFields = $pageObject->searchClauseObj->getSearchCtrlParams("need_hourly");
	$firstFieldParams = array();
	if (count($srchFields))
	{
		$firstFieldParams = $srchFields[0];
	...
	{
		$firstFieldParams['fName'] = "need_hourly";
		$firstFieldParams['eType'] = '';
		$firstFieldParams['value1'] = '';
		$firstFieldParams['opt'] = '';
		$firstFieldParams['value2'] = '';
	...
	if(isEnableSection508())
		$xt->assign_section("need_hourly_label","<label for=\"".GetInputElementId("need_hourly", $id, PAGE_SEARCH)."\">","</label>");
	else 
		$xt->assign("need_hourly_label", true);
	
	$xt->assign("need_hourly_fieldblock", true);
	$xt->assignbyref("need_hourly_editcontrol", $ctrlBlockArr['searchcontrol']);
	$xt->assign("need_hourly_notbox", $ctrlBlockArr['notbox']);
	// create second control, if need it
	$xt->assignbyref("need_hourly_editcontrol1", $ctrlBlockArr['searchcontrol1']);
	// create search type select
	$xt->assign("searchtype_need_hourly", $ctrlBlockArr['searchtype']);
	$isFieldNeedSecCtrl = $searchControlBuilder->isNeedSecondCtrl("need_hourly");
	$ctrlInd = 0;
	if ($isFieldNeedSecCtrl)
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"need_hourly", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd, 1=>($ctrlInd+1)));
		$ctrlInd+=2;
	}
	else
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"need_hourly", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd));			
		$ctrlInd++;
	}
	// search fields data
