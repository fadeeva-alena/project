...
if(isset($_REQUEST['afteradd'])){
		header('Location: _site_mode_add.php');
...
//	processing Message - start
	...
	if($inlineAddOption)
	{
		...
		$control_Message->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
...
//	processing Mode - start
	...
	if($inlineAddOption)
	{
		...
		$control_Mode->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing Mode - end
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'_site_mode_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'_site_mode_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: _site_mode_".$pageObject->getPageType().".php");
