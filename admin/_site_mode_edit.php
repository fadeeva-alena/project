...
$layout->blocks["top"][] = "details";$page_layouts["_site_mode_edit"] = $layout;
...
$templatefile = ($inlineedit == EDIT_INLINE) ? "_site_mode_inline_edit.htm" : "_site_mode_edit.htm";
...
//	processing Message - begin
	$condition = 1;

	if($condition)
	{
		$control_Message = $pageObject->getControl("Message", $id);
		$control_Message->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing Message - end
//	processing Mode - begin
	$condition = 1;

	if($condition)
	{
		$control_Mode = $pageObject->getControl("Mode", $id);
		$control_Mode->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);
...
//	processing Mode - end
	...
	//	processing Status - end
			//	processing Message - begin
				...
				if($condition)
				{
					$control_Message->afterSuccessfulSave();
				}
	//	processing Message - end
			//	processing Mode - begin
							$condition = 1;
				...
				if($condition)
				{
					$control_Mode->afterSuccessfulSave();
				}
	//	processing Mode - end
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// redirect
	header("Location: _site_mode_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: _site_mode_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($inlineedit != EDIT_INLINE)
{
	...
	if(!$pageObject->isAppearOnTabs("Message"))
		$xt->assign("Message_fieldblock",true);
	else
		$xt->assign("Message_tabfieldblock",true);
	$xt->assign("Message_label",true);
	if(isEnableSection508())
		$xt->assign_section("Message_label","<label for=\"".GetInputElementId("Message", $id, PAGE_EDIT)."\">","</label>");
		
	if(!$pageObject->isAppearOnTabs("Mode"))
		$xt->assign("Mode_fieldblock",true);
	else
		$xt->assign("Mode_tabfieldblock",true);
	$xt->assign("Mode_label",true);
	if(isEnableSection508())
		$xt->assign_section("Mode_label","<label for=\"".GetInputElementId("Mode", $id, PAGE_EDIT)."\">","</label>");
...
//	Message - 
	$value = $pageObject->showDBValue("Message", $data, $keylink);
	$showValues["Message"] = $value;
	$showFields[] = "Message";
		$showRawValues["Message"] = substr($data["Message"],0,100);

//	Mode - 
	$value = $pageObject->showDBValue("Mode", $data, $keylink);
	$showValues["Mode"] = $value;
	$showFields[] = "Mode";
		$showRawValues["Mode"] = substr($data["Mode"],0,100);
...
if($pageObject->isShowDetailTables && $inlineedit!=EDIT_INLINE && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "_site_mode";
		$options['firstTime'] = 1;
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='_site_mode_view.php?".$viewlink."'\"");
