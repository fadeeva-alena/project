...
$layout->blocks["top"][] = "export";$page_layouts["t_skills_export"] = $layout;
...
//	process selection
	...
	if (@$_REQUEST["mdelete"])
	{
		...
		{
			$keys=array();
			$keys["skill_id"] = refine($_REQUEST["mdelete1"][mdeleteIndex($ind)]);
			$selected_recs[] = $keys;
		}
	}
...
$xt->display("t_skills_export.htm");
...
function ExportToXML($cipherer)
{
	...
	{
		$values = array();
			$values["skill_id"] = $pageObject->showDBValue("skill_id", $row);
			$values["people_id"] = $pageObject->showDBValue("people_id", $row);
			$values["skill_type_id"] = $pageObject->showDBValue("skill_type_id", $row);
			$values["skill_subtype_id"] = $pageObject->showDBValue("skill_subtype_id", $row);
			$values["skill_note"] = $pageObject->showDBValue("skill_note", $row);
			$values["skill_hourly"] = $pageObject->showDBValue("skill_hourly", $row);
...
// write data rows
	...
	{
		$values = array();
			$values["skill_id"] = $pageObject->getViewControl("skill_id")->showDBValue($row, "");
			$values["people_id"] = $pageObject->getViewControl("people_id")->showDBValue($row, "");
			$values["skill_type_id"] = $pageObject->getViewControl("skill_type_id")->showDBValue($row, "");
			$values["skill_subtype_id"] = $pageObject->getViewControl("skill_subtype_id")->showDBValue($row, "");
			$values["skill_note"] = $pageObject->getViewControl("skill_note")->showDBValue($row, "");
			$values["skill_hourly"] = $pageObject->getViewControl("skill_hourly")->showDBValue($row, "");
		...
		if ($eventRes)
		{
			...
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["skill_id"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["people_id"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["skill_type_id"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["skill_subtype_id"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["skill_note"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["skill_hourly"]).'"';
			if($outstr!="")
				$outstr.=",";
...
// write header
	...
	{
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Skill Id").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("People Id").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Skill Type Id").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Skill Subtype Id").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Skill Note").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Skill Hourly").'</td>';	
	...
	else
	{
		...
		echo "<td>"."Skill Type Id"."</td>";
		echo "<td>"."Skill Subtype Id"."</td>";
		echo "<td>"."Skill Note"."</td>";
		echo "<td>"."Skill Hourly"."</td>";
		...
		if ($eventRes)
		{
			...
			echo '</td>';
							echo '<td>';
								...
								if($_REQUEST["type"]=="excel")
					echo PrepareForExcel($values["skill_type_id"]);
				else
					echo $values["skill_type_id"];//echo htmlspecialchars($values["skill_type_id"]); commented for bug #6823
			...
			echo '</td>';
							echo '<td>';
								...
								if($_REQUEST["type"]=="excel")
					echo PrepareForExcel($values["skill_subtype_id"]);
				else
					echo $values["skill_subtype_id"];//echo htmlspecialchars($values["skill_subtype_id"]); commented for bug #6823
			...
			echo '</td>';
					...
					echo '<td>';
									...
									if($_REQUEST["type"]=="excel")
						echo PrepareForExcel($values["skill_note"]);
					else
						echo $values["skill_note"];
			echo '</td>';
							echo '<td>';
			
									echo $values["skill_hourly"];
			echo '</td>';
							echo '<td>';
