...
$smarty->display("t_needs_export.htm");
...
function ExportToXML()
{
	...
	{
		echo "<row>\r\n";
		$field=htmlspecialchars(XMLNameEncode("need_id"));
		echo "<".$field.">";
		echo htmlspecialchars(GetData($row,"need_id",""));
		echo "</".$field.">\r\n";
		$field=htmlspecialchars(XMLNameEncode("people_id"));
		echo "<".$field.">";
		echo htmlspecialchars(GetData($row,"people_id",""));
		echo "</".$field.">\r\n";
		$field=htmlspecialchars(XMLNameEncode("need_type_id"));
		echo "<".$field.">";
/*		
		if(strlen($row["need_type_id"]))
		{
			$strdata = make_db_value("need_type_id",$row["need_type_id"]);
			$LookupSQL="SELECT ";
					$LookupSQL.="`skilltype`";
			$LookupSQL.=" FROM `t_skill_types` WHERE `skilltype_id` = " . $strdata;
					LogInfo($LookupSQL);
			$rsLookup = db_query($LookupSQL,$conn);
			$lookupvalue=$row["need_type_id"];
			if($lookuprow=db_fetch_numarray($rsLookup))
				$lookupvalue=$lookuprow[0];
			echo htmlspecialchars(GetDataInt($lookupvalue,$row,"need_type_id", ""));
		}
*/		
		echo htmlspecialchars(DisplayLookupWizard("need_type_id",$row["need_type_id"],$row,"",MODE_EXPORT));
		
		echo "</".$field.">\r\n";
		$field=htmlspecialchars(XMLNameEncode("need_subtype_id"));
		echo "<".$field.">";
/*		
		if(strlen($row["need_subtype_id"]))
		{
			$strdata = make_db_value("need_subtype_id",$row["need_subtype_id"]);
			$LookupSQL="SELECT ";
					$LookupSQL.="`skill_subtype`";
			$LookupSQL.=" FROM `t_skill_subtype` WHERE `skill_subtype_id` = " . $strdata;
					LogInfo($LookupSQL);
			$rsLookup = db_query($LookupSQL,$conn);
			$lookupvalue=$row["need_subtype_id"];
			if($lookuprow=db_fetch_numarray($rsLookup))
				$lookupvalue=$lookuprow[0];
			echo htmlspecialchars(GetDataInt($lookupvalue,$row,"need_subtype_id", ""));
		}
*/		
		echo htmlspecialchars(DisplayLookupWizard("need_subtype_id",$row["need_subtype_id"],$row,"",MODE_EXPORT));
		
		echo "</".$field.">\r\n";
		$field=htmlspecialchars(XMLNameEncode("need_note"));
		echo "<".$field.">";
		echo htmlspecialchars(GetData($row,"need_note",""));
		echo "</".$field.">\r\n";
		$field=htmlspecialchars(XMLNameEncode("need_hourly"));
		echo "<".$field.">";
		echo htmlspecialchars(GetData($row,"need_hourly",""));
		echo "</".$field.">\r\n";
...
// write data rows
	...
	while((!$nPageSize || $iNumberOfRows<$nPageSize) && $row)
	{
		...
		if($outstr!="")
			$outstr.=",";
			$format="";
		$outstr.='"'.htmlspecialchars(GetData($row,"need_id",$format)).'"';
		if($outstr!="")
			$outstr.=",";
			$format="";
...
/*
		if(strlen($row["need_type_id"]))
		{
			...
			if($lookuprow=db_fetch_numarray($rsLookup))
				$lookupvalue=$lookuprow[0];
			$outstr.='"'.htmlspecialchars(GetDataInt($lookupvalue,$row,"need_type_id", "")).'"';
		}
*/		
		$value = DisplayLookupWizard("need_type_id",$row["need_type_id"],$row,"",MODE_EXPORT);
		if(strlen($value))
...
/*
		if(strlen($row["need_subtype_id"]))
		{
			...
			if($lookuprow=db_fetch_numarray($rsLookup))
				$lookupvalue=$lookuprow[0];
			$outstr.='"'.htmlspecialchars(GetDataInt($lookupvalue,$row,"need_subtype_id", "")).'"';
		}
*/		
		$value = DisplayLookupWizard("need_subtype_id",$row["need_subtype_id"],$row,"",MODE_EXPORT);
		if(strlen($value))
			$outstr.='"'.htmlspecialchars($value).'"';
		...
		if($outstr!="")
			$outstr.=",";
			$format="";
		$outstr.='"'.htmlspecialchars(GetData($row,"need_note",$format)).'"';
		if($outstr!="")
			$outstr.=",";
			$format="";
		$outstr.='"'.htmlspecialchars(GetData($row,"need_hourly",$format)).'"';
		if($outstr!="")
			$outstr.=",";
			$format="";
...
// write header
	...
	if($_REQUEST["type"]=="excel")
	{
		echo '<td style="width: 100" x:str>'.PrepareForExcel("need_id").'</td>';
		echo '<td style="width: 100" x:str>'.PrepareForExcel("people_id").'</td>';
		echo '<td style="width: 100" x:str>'.PrepareForExcel("need_type_id").'</td>';
		echo '<td style="width: 100" x:str>'.PrepareForExcel("need_subtype_id").'</td>';
		echo '<td style="width: 100" x:str>'.PrepareForExcel("need_note").'</td>';
		echo '<td style="width: 100" x:str>'.PrepareForExcel("need_hourly").'</td>';
	...
	else
	{
		...
		echo "<td>need_type_id</td>";
		echo "<td>need_subtype_id</td>";
		echo "<td>need_note</td>";
		echo "<td>need_hourly</td>";
...
// write data rows
	...
	echo '<td>';
		...
		$format="";
			echo htmlspecialchars(GetData($row,"need_id",$format));
	echo '</td>';
	echo '<td>';
...
/*
			...
			$strValue=GetDataInt($lookupvalue,$row,"need_type_id", "");
*/			
			$strValue = DisplayLookupWizard("need_type_id",$row["need_type_id"],$row,"",MODE_EXPORT);
						if($_REQUEST["type"]=="excel")
...
/*
			...
			$strValue=GetDataInt($lookupvalue,$row,"need_subtype_id", "");
*/			
			$strValue = DisplayLookupWizard("need_subtype_id",$row["need_subtype_id"],$row,"",MODE_EXPORT);
						if($_REQUEST["type"]=="excel")
				echo PrepareForExcel($strValue);
			else
				echo htmlspecialchars($strValue);
	...
	if($_REQUEST["type"]=="excel")
		...
		$format="";
			if($_REQUEST["type"]=="excel")
			echo PrepareForExcel(GetData($row,"need_note",$format));
		else
			echo htmlspecialchars(GetData($row,"need_note",$format));
	echo '</td>';
	echo '<td>';

		$format="";
			echo htmlspecialchars(GetData($row,"need_hourly",$format));
	echo '</td>';
	echo '<td>';
...
function ExportToPDF()
{
	...
	function AcceptPageBreak()
	{
		...
		$this->Rect($x,$this->y0,$colwidth["need_id"],$this->maxheight);
		$x+=$colwidth["need_id"];
		$this->Rect($x,$this->y0,$colwidth["people_id"],$this->maxheight);
		$x+=$colwidth["people_id"];
		$this->Rect($x,$this->y0,$colwidth["need_type_id"],$this->maxheight);
		$x+=$colwidth["need_type_id"];
		$this->Rect($x,$this->y0,$colwidth["need_subtype_id"],$this->maxheight);
		$x+=$colwidth["need_subtype_id"];
		$this->Rect($x,$this->y0,$colwidth["need_note"],$this->maxheight);
		$x+=$colwidth["need_note"];
		$this->Rect($x,$this->y0,$colwidth["need_hourly"],$this->maxheight);
		$x+=$colwidth["need_hourly"];
...
//	draw frame	
	...
	function Header()
	{
		...
		$this->Cell($colwidth["need_id"],$this->rowheight,"need_id",1,0,'C',1);
		$this->Cell($colwidth["people_id"],$this->rowheight,"people_id",1,0,'C',1);
		$this->Cell($colwidth["need_type_id"],$this->rowheight,"need_type_id",1,0,'C',1);
		$this->Cell($colwidth["need_subtype_id"],$this->rowheight,"need_subtype_id",1,0,'C',1);
		$this->Cell($colwidth["need_note"],$this->rowheight,"need_note",1,0,'C',1);
		$this->Cell($colwidth["need_hourly"],$this->rowheight,"need_hourly",1,0,'C',1);
	...
	$colwidth=array();
    $colwidth["need_id"]=$defwidth;
    $colwidth["people_id"]=$defwidth;
    $colwidth["need_type_id"]=$defwidth;
    $colwidth["need_subtype_id"]=$defwidth;
    $colwidth["need_note"]=$defwidth;
    $colwidth["need_hourly"]=$defwidth;
	...
	while((!$nPageSize || $i<$nPageSize) && $row)
	{
		...
		$pdf->SetY($pdf->y0);
		$pdf->SetX($x);
		$pdf->MultiCell($colwidth["need_id"],$rowheight,GetData($row,"need_id",""));
		$x+=$colwidth["need_id"];
...
/*			$strdata = make_db_value("need_type_id",$row["need_type_id"]);
			...
			if($lookuprow=db_fetch_numarray($rsLookup))
				$lookupvalue=$lookuprow[0];
			$pdf->Cell($colwidth["need_type_id"],$rowheight,GetDataInt($lookupvalue,$row,"need_type_id", ""));
*/				
				
			$value = DisplayLookupWizard("need_type_id",$row["need_type_id"],$row,"",MODE_EXPORT);
			$pdf->Cell($colwidth["need_type_id"],$rowheight,$value);
		}
		$x+=$colwidth["need_type_id"];
		if($pdf->GetY()-$pdf->y0>$pdf->maxheight)
			$pdf->maxheight=$pdf->GetY()-$pdf->y0;
...
/*			$strdata = make_db_value("need_subtype_id",$row["need_subtype_id"]);
			...
			if($lookuprow=db_fetch_numarray($rsLookup))
				$lookupvalue=$lookuprow[0];
			$pdf->Cell($colwidth["need_subtype_id"],$rowheight,GetDataInt($lookupvalue,$row,"need_subtype_id", ""));
*/				
				
			$value = DisplayLookupWizard("need_subtype_id",$row["need_subtype_id"],$row,"",MODE_EXPORT);
			$pdf->Cell($colwidth["need_subtype_id"],$rowheight,$value);
		}
		$x+=$colwidth["need_subtype_id"];
		if($pdf->GetY()-$pdf->y0>$pdf->maxheight)
			$pdf->maxheight=$pdf->GetY()-$pdf->y0;
		$pdf->SetY($pdf->y0);
		$pdf->SetX($x);
		$pdf->MultiCell($colwidth["need_note"],$rowheight,GetData($row,"need_note",""));
		$x+=$colwidth["need_note"];
		if($pdf->GetY()-$pdf->y0>$pdf->maxheight)
			$pdf->maxheight=$pdf->GetY()-$pdf->y0;
		$pdf->SetY($pdf->y0);
		$pdf->SetX($x);
		$pdf->MultiCell($colwidth["need_hourly"],$rowheight,GetData($row,"need_hourly",""));
		$x+=$colwidth["need_hourly"];
		if($pdf->GetY()-$pdf->y0>$pdf->maxheight)
			$pdf->maxheight=$pdf->GetY()-$pdf->y0;
		$pdf->SetY($pdf->y0);
...
//	draw fames
		$x=$leftmargin;
		$pdf->Rect($x,$pdf->y0,$colwidth["need_id"],$pdf->maxheight);
		$x+=$colwidth["need_id"];
		$pdf->Rect($x,$pdf->y0,$colwidth["people_id"],$pdf->maxheight);
		$x+=$colwidth["people_id"];
		$pdf->Rect($x,$pdf->y0,$colwidth["need_type_id"],$pdf->maxheight);
		$x+=$colwidth["need_type_id"];
		$pdf->Rect($x,$pdf->y0,$colwidth["need_subtype_id"],$pdf->maxheight);
		$x+=$colwidth["need_subtype_id"];
		$pdf->Rect($x,$pdf->y0,$colwidth["need_note"],$pdf->maxheight);
		$x+=$colwidth["need_note"];
		$pdf->Rect($x,$pdf->y0,$colwidth["need_hourly"],$pdf->maxheight);
		$x+=$colwidth["need_hourly"];
