...
//	validation stuff
	...
	$includes.="<script type=\"text/javascript\" src=\"include/inlineedit.js\"></script>\r\n";
		...
		$editValidateFields[] = "Status";
			$editValidateTypes[] = "";
		$editValidateFields[] = "Message";
			$editValidateTypes[] = "";
		$editValidateFields[] = "Mode";
	
			$addValidateTypes[] = "";
		$addValidateFields[] = "Status";
			$addValidateTypes[] = "";
		$addValidateFields[] = "Message";
			$addValidateTypes[] = "";
		$addValidateFields[] = "Mode";
...
if ($useAJAX) {
	$includes.="var INLINE_EDIT_TABLE='_site_mode_edit.php';\r\n";
	$includes.="var INLINE_ADD_TABLE='_site_mode_add.php';\r\n";
	$includes.="var INLINE_VIEW_TABLE='_site_mode_view.php';\r\n";
	$includes.="var SUGGEST_TABLE='_site_mode_searchsuggest.php';\r\n";
	$includes.="var MASTER_PREVIEW_TABLE='_site_mode_masterpreview.php';\r\n";
}
$includes.="\n</script>\n";
if ($useAJAX) {
...
$smarty->assign("order_dir_Status","a");
$smarty->assign("order_dir_Message","a");
$smarty->assign("order_dir_Mode","a");
...
if(@$_SESSION[$strTableName."_orderby"])
{
	...
	if($order_field=="Status")
	{
		if($order_dir=="a")
		{
			$smarty->assign("order_dir_Status","d");
			$img="up";
		}
		else
			$img="down";
		$smarty->assign("order_image_Status","<img src=\"images/".$img.".gif\" border=0>");
	}
	$smarty->assign("order_dir_Message","a");
	if($order_field=="Message")
	{
		if($order_dir=="a")
		{
			$smarty->assign("order_dir_Message","d");
			$img="up";
		}
		else
			$img="down";
		$smarty->assign("order_image_Message","<img src=\"images/".$img.".gif\" border=0>");
	}
	$smarty->assign("order_dir_Mode","a");
	if($order_field=="Mode")
	{
		if($order_dir=="a")
		{
			$smarty->assign("order_dir_Mode","d");
			$img="up";
		}
		else
			$img="down";
		$smarty->assign("order_image_Mode","<img src=\"images/".$img.".gif\" border=0>");
	}
	$smarty->assign("order_dir_ID","a");
	if($order_field=="ID")
	{
...
//			$strSQL = AddWhere($strSQL,"1=0");
	...
	else
	{
		...
		if($where=StrWhere("Status", $strSearchFor, $strSearchOption, ""))
			$strWhere .= " or ".$where;
		if($where=StrWhere("Message", $strSearchFor, $strSearchOption, ""))
			$strWhere .= " or ".$where;
		if($where=StrWhere("Mode", $strSearchFor, $strSearchOption, ""))
			$strWhere .= " or ".$where;
...
$smarty->assign("pagination","<script language=\"JavaScript\">WritePagination(".$mypage.",".$maxpages.");
		...
		{
			window.location='_site_mode_list.php?goto='+nPageNumber;
		}
</script>");
...
//	Message - 
			$value="";
				$value = ProcessLargeText(GetData($data,"Message", ""),"field=Message".$keylink,"",MODE_LIST);
			$row[$col."Message_value"]=$value;

//	Mode - 
			$value="";
				$value = ProcessLargeText(GetData($data,"Mode", ""),"field=Mode".$keylink,"",MODE_LIST);
...
//	ID - 
			...
			$span="<span ";
			$span.="id=\"edit".$recno."_Status\" ";
					$span.=">";
			$row[$col."Status_value"] = $span.$row[$col."Status_value"]."</span>";
			$span="<span ";
			$span.="id=\"edit".$recno."_Message\" ";
					$span.=">";
			$row[$col."Message_value"] = $span.$row[$col."Message_value"]."</span>";
			$span="<span ";
			$span.="id=\"edit".$recno."_Mode\" ";
					$span.=">";
			$row[$col."Mode_value"] = $span.$row[$col."Mode_value"]."</span>";
			$span="<span ";
...
//	fill in search variables
	//	field selection
		if(@$_SESSION[$strTableName."_searchfield"]=="Status")
			$smarty->assign("search_Status","selected");
		if(@$_SESSION[$strTableName."_searchfield"]=="Message")
			$smarty->assign("search_Message","selected");
		if(@$_SESSION[$strTableName."_searchfield"]=="Mode")
			$smarty->assign("search_Mode","selected");
...
$templatefile = "_site_mode_list.htm";
