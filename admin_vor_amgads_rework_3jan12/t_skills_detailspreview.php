...
if ( $rowcount ) {
	...
	echo "<td><strong>skill_id</strong></td>";
	echo "<td><strong>people_id</strong></td>";
	echo "<td><strong>skill_type_id</strong></td>";
	echo "<td><strong>skill_subtype_id</strong></td>";
	echo "<td><strong>skill_note</strong></td>";
	echo "<td><strong>skill_hourly</strong></td>";
	...
	//	skill_id - 
		    $value="";
				$value = ProcessLargeText(GetData($data,"skill_id", ""),"field=skill%5Fid".$keylink,"",MODE_PRINT);
			echo "<td>".$value."</td>";
	//	people_id - 
		    $value="";
	...
	/*	
			if(strlen($data["skill_type_id"]))
			{
				...
				$value=ProcessLargeText(GetDataInt($lookupvalue,$data,"skill_type_id", ""),"field=skill%5Ftype%5Fid".$keylink,"",MODE_PRINT);
			}
			else
				$value="";
*/				
			$value=DisplayLookupWizard("skill_type_id",$data["skill_type_id"],$data,$keylink,MODE_PRINT);
	...
	/*	
			if(strlen($data["skill_subtype_id"]))
			{
				...
				if($lookuprow=db_fetch_numarray($rsLookup))
					$lookupvalue=$lookuprow[0];
				$value=ProcessLargeText(GetDataInt($lookupvalue,$data,"skill_subtype_id", ""),"field=skill%5Fsubtype%5Fid".$keylink,"",MODE_PRINT);
			}
			else
				$value="";
*/				
			$value=DisplayLookupWizard("skill_subtype_id",$data["skill_subtype_id"],$data,$keylink,MODE_PRINT);
			
			echo "<td>".$value."</td>";
	//	skill_note - 
		    $value="";
				$value = ProcessLargeText(GetData($data,"skill_note", ""),"field=skill%5Fnote".$keylink,"",MODE_PRINT);
			echo "<td>".$value."</td>";
	//	skill_hourly - 
		    $value="";
				$value = ProcessLargeText(GetData($data,"skill_hourly", ""),"field=skill%5Fhourly".$keylink,"",MODE_PRINT);
			echo "<td>".$value."</td>";
	//	prof_provider - 
		    $value="";
