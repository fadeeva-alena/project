...
if ($useAJAX) {
	$includes.="var INLINE_EDIT_TABLE='t_needs_edit.php';\r\n";
	$includes.="var INLINE_ADD_TABLE='t_needs_add.php';\r\n";
	$includes.="var INLINE_VIEW_TABLE='t_needs_view.php';\r\n";
	$includes.="var SUGGEST_TABLE='t_needs_searchsuggest.php';\r\n";
	$includes.="var MASTER_PREVIEW_TABLE='t_needs_masterpreview.php';\r\n";
}
$includes.="\n</script>\n";
if ($useAJAX) {
...
$smarty->assign("order_dir_need_id","a");
$smarty->assign("order_dir_people_id","a");
$smarty->assign("order_dir_need_type_id","a");
$smarty->assign("order_dir_need_subtype_id","a");
$smarty->assign("order_dir_need_note","a");
$smarty->assign("order_dir_need_hourly","a");
...
if(@$_SESSION[$strTableName."_orderby"])
{
	...
	$smarty->assign("order_dir_need_id","a");
	if($order_field=="need_id")
	{
		if($order_dir=="a")
		{
			$smarty->assign("order_dir_need_id","d");
			$img="up";
		}
		else
			$img="down";
		$smarty->assign("order_image_need_id","<img src=\"images/".$img.".gif\" border=0>");
	}
	$smarty->assign("order_dir_people_id","a");
	...
	$smarty->assign("order_dir_need_type_id","a");
	if($order_field=="need_type_id")
	{
		if($order_dir=="a")
		{
			$smarty->assign("order_dir_need_type_id","d");
			$img="up";
		}
		else
			$img="down";
		$smarty->assign("order_image_need_type_id","<img src=\"images/".$img.".gif\" border=0>");
	}
	$smarty->assign("order_dir_need_subtype_id","a");
	if($order_field=="need_subtype_id")
	{
		if($order_dir=="a")
		{
			$smarty->assign("order_dir_need_subtype_id","d");
			$img="up";
		}
		else
			$img="down";
		$smarty->assign("order_image_need_subtype_id","<img src=\"images/".$img.".gif\" border=0>");
	}
	$smarty->assign("order_dir_need_note","a");
	if($order_field=="need_note")
	{
		if($order_dir=="a")
		{
			$smarty->assign("order_dir_need_note","d");
			$img="up";
		}
		else
			$img="down";
		$smarty->assign("order_image_need_note","<img src=\"images/".$img.".gif\" border=0>");
	}
	$smarty->assign("order_dir_need_hourly","a");
	if($order_field=="need_hourly")
	{
		if($order_dir=="a")
		{
			$smarty->assign("order_dir_need_hourly","d");
			$img="up";
		}
		else
			$img="down";
		$smarty->assign("order_image_need_hourly","<img src=\"images/".$img.".gif\" border=0>");
	}
	$smarty->assign("order_dir_prof_provider","a");
...
//			$strSQL = AddWhere($strSQL,"1=0");
	...
	{
		$strWhere = "1=0";
		if($where=StrWhere("need_id", $strSearchFor, $strSearchOption, ""))
			$strWhere .= " or ".$where;
		if($where=StrWhere("people_id", $strSearchFor, $strSearchOption, ""))
			$strWhere .= " or ".$where;
		if($where=StrWhere("need_type_id", $strSearchFor, $strSearchOption, ""))
			$strWhere .= " or ".$where;
		if($where=StrWhere("need_subtype_id", $strSearchFor, $strSearchOption, ""))
			$strWhere .= " or ".$where;
		if($where=StrWhere("need_note", $strSearchFor, $strSearchOption, ""))
			$strWhere .= " or ".$where;
		if($where=StrWhere("need_hourly", $strSearchFor, $strSearchOption, ""))
			$strWhere .= " or ".$where;
...
$smarty->assign("pagination","<script language=\"JavaScript\">WritePagination(".$mypage.",".$maxpages.");
		...
		{
			window.location='t_needs_list.php?goto='+nPageNumber;
		}
</script>");
...
//	key fields
			$keyblock="";
			$row[$col."id1"]=htmlspecialchars($data["need_id"]);
			$keyblock.= rawurlencode($data["need_id"]);
			$row[$col."keyblock"]=htmlspecialchars($keyblock);
			$row[$col."recno"] = $recno;
//	detail tables
//	edit page link
			$editlink="";
			$editlink.="editid1=".htmlspecialchars(rawurlencode($data["need_id"]));
			$row[$col."editlink"]=$editlink;

			$copylink="";
			$copylink.="copyid1=".htmlspecialchars(rawurlencode($data["need_id"]));
...
//	need_id - 
			$value="";
				$value = ProcessLargeText(GetData($data,"need_id", ""),"field=need%5Fid".$keylink,"",MODE_LIST);
			...
			if(strlen($data["need_type_id"]))
			{
				...
				if($lookuprow=db_fetch_numarray($rsLookup))
					$lookupvalue=$lookuprow[0];
								$value=ProcessLargeText(GetDataInt($lookupvalue,$data,"need_type_id", ""),"field=need%5Ftype%5Fid".$keylink,"",MODE_LIST);
			}
			else
				$value="";
*/			
			$value=DisplayLookupWizard("need_type_id",$data["need_type_id"],$data,$keylink,MODE_LIST);
...
//	need_subtype_id - 
	...
	/*	
			if(strlen($data["need_subtype_id"]))
			{
				...
				if($lookuprow=db_fetch_numarray($rsLookup))
					$lookupvalue=$lookuprow[0];
								$value=ProcessLargeText(GetDataInt($lookupvalue,$data,"need_subtype_id", ""),"field=need%5Fsubtype%5Fid".$keylink,"",MODE_LIST);
			}
			else
				$value="";
*/			
			$value=DisplayLookupWizard("need_subtype_id",$data["need_subtype_id"],$data,$keylink,MODE_LIST);
			$row[$col."need_subtype_id_value"]=$value;

//	need_note - 
			$value="";
				$value = ProcessLargeText(GetData($data,"need_note", ""),"field=need%5Fnote".$keylink,"",MODE_LIST);
			$row[$col."need_note_value"]=$value;

//	need_hourly - 
			$value="";
				$value = ProcessLargeText(GetData($data,"need_hourly", ""),"field=need%5Fhourly".$keylink,"",MODE_LIST);
...
//	fill in search variables
	//	field selection
		if(@$_SESSION[$strTableName."_searchfield"]=="need_id")
			$smarty->assign("search_need_id","selected");
		if(@$_SESSION[$strTableName."_searchfield"]=="people_id")
			$smarty->assign("search_people_id","selected");
		if(@$_SESSION[$strTableName."_searchfield"]=="need_type_id")
			$smarty->assign("search_need_type_id","selected");
		if(@$_SESSION[$strTableName."_searchfield"]=="need_subtype_id")
			$smarty->assign("search_need_subtype_id","selected");
		if(@$_SESSION[$strTableName."_searchfield"]=="need_note")
			$smarty->assign("search_need_note","selected");
		if(@$_SESSION[$strTableName."_searchfield"]=="need_hourly")
			$smarty->assign("search_need_hourly","selected");
