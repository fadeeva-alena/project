...
// long binary data?
	...
	else if($format==FORMAT_FILE)
	{
		...
		if($strTableName=="_taccess")
		{
			$iquery.="&key1=".rawurlencode($data["ID"]);
		}
		if($strTableName=="_site_mode")
		{
			$iquery.="&key1=".rawurlencode($data["ID"]);
		}
		if($strTableName=="t_log")
		{
			$iquery.="&key1=".rawurlencode($data["log_id"]);
		}
		if($strTableName=="_t_search")
		{
			$iquery.="&key1=".rawurlencode($data["searchid"]);
		}
		if($strTableName=="t_people")
		{
			$iquery.="&key1=".rawurlencode($data["people_id"]);
		}
		if($strTableName=="t_skills")
		{
			$iquery.="&key1=".rawurlencode($data["skill_id"]);
		}
		if($strTableName=="t_needs")
		{
			$iquery.="&key1=".rawurlencode($data["need_id"]);
		}
...
//	t_log
	if($table=="t_log")
	{
			$value=make_db_value("log_id",$keys["log_id"]);
				if($value==="null")
			$strWhere.=GetFullFieldName("log_id")." is null";
		else
			$strWhere.=GetFullFieldName("log_id")."=".make_db_value("log_id",$keys["log_id"]);
	}

//	_t_search
	if($table=="_t_search")
	{
			$value=make_db_value("searchid",$keys["searchid"]);
				if($value==="null")
			$strWhere.=GetFullFieldName("searchid")." is null";
		else
			$strWhere.=GetFullFieldName("searchid")."=".make_db_value("searchid",$keys["searchid"]);
	}

//	t_people
	if($table=="t_people")
	{
			$value=make_db_value("people_id",$keys["people_id"]);
				if($value==="null")
			$strWhere.=GetFullFieldName("people_id")." is null";
		else
			$strWhere.=GetFullFieldName("people_id")."=".make_db_value("people_id",$keys["people_id"]);
	}

//	t_skills
	if($table=="t_skills")
	{
			$value=make_db_value("skill_id",$keys["skill_id"]);
				if($value==="null")
			$strWhere.=GetFullFieldName("skill_id")." is null";
		else
			$strWhere.=GetFullFieldName("skill_id")."=".make_db_value("skill_id",$keys["skill_id"]);
	}

//	t_needs
	if($table=="t_needs")
	{
			$value=make_db_value("need_id",$keys["need_id"]);
				if($value==="null")
			$strWhere.=GetFullFieldName("need_id")." is null";
		else
			$strWhere.=GetFullFieldName("need_id")."=".make_db_value("need_id",$keys["need_id"]);
	}
	return $strWhere;
}
...
//	build SQL strings for lookup wizards
		if($strTableName=="_taccess" && $field=="Zip") 
		{
				$LinkField="ZIP";
			$DisplayField="ZIP";
			$LookupTable="t_zipch";
			...
			$LookupSQL .= "`ZIP`";
						...
						$LookupSQL.=" ORDER BY `t_zipch`.`ZIP`";
			
		}
		if($strTableName=="_taccess" && $field=="Location") 
		{
				$LinkField="Location";
		...
		if($strTableName=="_t_search" && $field=="Skill_type") 
		{
				$LinkField="skilltype_id";
			$DisplayField="skilltype";
			$LookupTable="t_skill_types";
			$strSize=1;
		...
		if($strTableName=="_t_search" && $field=="Skill_Subtype") 
		{
				$LinkField="skill_subtype_id";
			$DisplayField="skill_subtype";
			$LookupTable="t_skill_subtype";
			$strSize=1;
		...
		if($strTableName=="t_skills" && $field=="skill_type_id") 
		{
				$LinkField="skilltype_id";
			$DisplayField="skilltype";
			$LookupTable="t_skill_types";
		...
		if($strTableName=="t_skills" && $field=="skill_subtype_id") 
		{
				$LinkField="skill_subtype_id";
			$DisplayField="skill_subtype";
			$LookupTable="t_skill_subtype";
		...
		if($strTableName=="t_needs" && $field=="need_type_id") 
		{
				$LinkField="skilltype_id";
			$DisplayField="skilltype";
			$LookupTable="t_skill_types";
		...
		if($strTableName=="t_needs" && $field=="need_subtype_id") 
		{
				$LinkField="skill_subtype_id";
			$DisplayField="skill_subtype";
			$LookupTable="t_skill_subtype";
...
//	dependent dropdown
				...
				$onchange="";
				if($strTableName=="_taccess" && $field == "Zip") 
					$onchange .= "loadSelectContent('_taccess','Zip', 'Location', '".$id."');";
		   		if($onchange)
...
//	get the initial value
			...
			if($strTableName=="_taccess" && $field=="Zip") 
			{
				...
				$lookup_SQL .= "`ZIP`";
								$lookup_SQL .= ",`ZIP`";
				$lookup_SQL .= " FROM `t_zipch` ";
								$lookup_SQL .= " WHERE `ZIP`=".make_db_value($field,$value)."";
									$lookup_SQL.= " ORDER BY `ZIP`";
								}
			if($strTableName=="_taccess" && $field=="Location") 
			{
								$lookup_SQL = "SELECT ";
								...
								$lookup_SQL .= ",`Location`";
				$lookup_SQL .= " FROM `t_zipch` ";
								$lookup_SQL .= " WHERE `Location`=".make_db_value($field,$value)."";
							}
			if($strTableName=="_t_search" && $field=="Skill_type") 
			{
								$lookup_SQL = "SELECT ";
								$lookup_SQL .= "`skilltype_id`";
								$lookup_SQL .= ",`skilltype`";
				$lookup_SQL .= " FROM `t_skill_types` ";
								$lookup_SQL .= " WHERE `skilltype_id`=".make_db_value($field,$value)."";
							}
			if($strTableName=="_t_search" && $field=="Skill_Subtype") 
			{
								$lookup_SQL = "SELECT ";
								$lookup_SQL .= "`skill_subtype_id`";
								$lookup_SQL .= ",`skill_subtype`";
				$lookup_SQL .= " FROM `t_skill_subtype` ";
								$lookup_SQL .= " WHERE `skill_subtype_id`=".make_db_value($field,$value)."";
							}
			if($strTableName=="t_skills" && $field=="skill_type_id") 
			{
								$lookup_SQL = "SELECT ";
...
//	build the control
			...
			if($addnewitem &&  $mode!=MODE_SEARCH)
			{
				...
				{
					if( $mode==MODE_INLINE_EDIT || $mode==MODE_INLINE_ADD )
						echo "<a href=# onclick=\"return DisplayAdd(event,'".$add_page."','value_".GoodFieldName($field)."_".$id."','".GoodFieldName($field)."','".GetTableURL()."','');\">"."Add new"."</a>";
					else
						echo "<a href=# onclick=\"return DisplayAdd(event,'".$add_page."','value_".GoodFieldName($field)."','".GoodFieldName($field)."','".GetTableURL()."','');\">"."Add new"."</a>";
				}
			}
...
//	classic dropdown - start
			...
			if($strTableName=="_taccess" && $field == "Zip") 
				...
				{
					$onchange .= "SetSelection('".$cfield."', 'value_Location', this.options[this.selectedIndex].value, '', arr_Location);if(value_Location.onchange) value_Location.onchange();";
				}
	   		if($onchange)
				$onchange="onchange=\"".$onchange."\"";
			if($strTableName=="_taccess" && $field == "Location") 
			{
				...
				if (!$useAJAX) 
				{
					...
					BuildSecondDropdownArray("arr_Location", $LookupSQL);  
					echo "</script>";
				}
//	add new item
				if($addnewitem &&  $mode!=MODE_SEARCH)
				{
					if($mode!=MODE_INLINE_EDIT && $mode!=MODE_INLINE_ADD)
					{
						$celement="document.forms.editform.value_Zip";
						$extra="";
					}
					else
					{
						$celement="document.getElementById('value_Zip_".$id."')";
						$extra="&mode=".$mode."&id=value_Location_".$id;
					}
									$celementvalue = $celement.".options[".$celement.".selectedIndex].value";
					if(!$advancedadd)
					{
						echo "<a href=# onclick=\"window.open('_taccess_addnewitem.php?".
					...
					{
						echo "<a href=# onclick=\"return DisplayAdd(event,'".$add_page."','".htmlspecialchars(jsreplace($cfieldid))."','".GoodFieldName($field)."','".GetTableURL()."',".$celementvalue.");\">"."Add new"."</a>";
					}
				}
				return;
			}
...
//	add new item
			if($addnewitem &&  $mode!=MODE_SEARCH && $mode!=MODE_INLINE_EDIT && $mode!=MODE_INLINE_ADD)
			{
				...
				{
					echo "<a href=# onclick=\"return DisplayAdd(event,'".$add_page."','".htmlspecialchars(jsreplace($cfield))."','".GoodFieldName($field)."','".GetTableURL()."','');\">"."Add new"."</a>";
				}
			}
			if($addnewitem &&  $mode!=MODE_SEARCH &&  ($mode==MODE_INLINE_EDIT || $mode==MODE_INLINE_ADD))
			{
...
function BuildEditControl($field , $value, $format, $edit, $secondfield=false, $id="")
{
	...
	if($strTableName=="t_log")
	{
		$keylink.="&key1=".rawurlencode(@$keys["log_id"]);
		$iquery.=$keylink;
	}
	if($strTableName=="_t_search")
	{
		$keylink.="&key1=".rawurlencode(@$keys["searchid"]);
		$iquery.=$keylink;
	}
	if($strTableName=="t_people")
	{
		$keylink.="&key1=".rawurlencode(@$keys["people_id"]);
		$iquery.=$keylink;
	}
	if($strTableName=="t_skills")
	{
		$keylink.="&key1=".rawurlencode(@$keys["skill_id"]);
		$iquery.=$keylink;
	}
	if($strTableName=="t_needs")
	{
		$keylink.="&key1=".rawurlencode(@$keys["need_id"]);
		$iquery.=$keylink;
	}
...
function loadSelectContent($field, $value,$fvalue="")
{
	...
	if($strTableName=="_taccess" && $field=="Location") 
	{
		$value = make_db_value("Zip",$value);
			$LookupSQL = "SELECT ";
			$LookupSQL .= "`Location`";
			$LookupSQL .= ",`Location`";
		$LookupSQL .= " FROM `t_zipch` ";
			$LookupSQL .= " WHERE `ZIP`=".$value;
		}
	$rs=db_query($LookupSQL,$conn);
