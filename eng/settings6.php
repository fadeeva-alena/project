...
$monthNames = array("Januar", "Februar", "March", "April", "Mai", "Juni",   
...
<input type=button value="Legal" onclick=location.href="Rechtliches.php" id="helpbtn3">
	...
	<input type="button" value="My Settings"  onclick=location.href="Settings5.php" id="maindatbtn">
<?php
...
?>
  ...
  <div class="mainContent">
	<div class="content">
<span id="title">6)Legal aspects: <img src="../images/page6.png" border="0"  usemap="#green" align="right" /> </span>
<map name="green">
...
of the premium may be charged to the employee.</font></span></span></span></p>
	...
	<!--<p><input type="button" name="nextbtn" id="nextbtn" value="Weiter zum Angebotsprofil (3 von 5)" onclick=location.href="settings3.html" /></p>-->
			...
			<div Align="Center"><a href="http://www.manimano.ch/Agreement.html">
				Print</a> </div><br>
			<div Align="left">
<?php
                   if ($row['Agree'] == 1){                        
echo"<input type='checkbox' name='flagm1' value='flag' checked   Disabled>Read, understood and accepted, {$row['zip']} {$row['location']} {$row['firstname']} {$row['lastname']}";
}else{
echo"<input type='checkbox' name='flagm1' value='flag' onclick='javascript:Change( )();  ' >Read, understood and accepted, {$row['zip']} {$row['location']} {$row['firstname']} {$row['lastname']}";
...
echo "   $todayis   ";
echo "<br>";
echo"Your Data is being transferred. You will receive an email with a link - click on the link - then you login and start immediately. We wisch you lots of fun with ManiMano!";
}

?>
</div><br><input type="hidden" Name="Nv"  id="Nv" value="<?php echo"$Nv";?>">	
<?php
                   if ($row['Agree'] == 1){      
      echo"<input name='Speichern' type='submit' id='finishbtn' value='Save' >";
}else
{
echo"<input name='Speichern' type='submit' id='finishbtn' value='Save' Disabled >";
}
?>
		</form>
<script type="text/javascript">
