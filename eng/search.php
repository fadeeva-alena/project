...
<input type=button value="Legal" onclick=location.href="Rechtliches.php" id="helpbtn3">
	...
	<input type=button value="Search" id="maindatbtn">
...
?>
				...
				Looking for a job or help?<br />Here you can choose - please select one from below.
                ...
                Seeking help / I will pay for it</label> <br />
                	<label><input type="radio" name="RadioGroup1" value="2" id="RadioGroup1_1" /<?PHP print $type1_status; ?>>
                I would like to offer help / and make some money</label>
				</form></div>
                </li>
			<li>
<?php
...
while ($row = mysql_fetch_assoc($quer2)) {
           if($row['skilltype_id']==@$kinder){echo "<option selected value='$row[skilltype_id]'>$row[seng]</option>"."<BR>";
$image=$row['image_path'];}
else{echo '<option value="' . $row['skilltype_id'] . '" name="' . $row['seng']. '">' . $row['seng']. '</option>'; }
...
while ($row = mysql_fetch_assoc($quer)) {
			echo '<option value="' . $row['skill_subtype_id'] . '" name="' . $row['steng']. '">' . $row['steng']. '</option>';
		}
...
?>
				<p>In certain cases, the gender selection is important.<br />Here you can define what we see - the choice is optional.</p>
            ...
            not imported / not specified.</label>
					<img src="../images/no_picture_he_and_she.gif" width="78" height="53" align="middle" />
				</form></div></li>
			<li>
			<div><div id="li4">Search is always in your<br />
			        surrounding area (about 5min/500m).<br />
			        Press SEARCH to start.</div><input type="button" name="button" id="searchbtn" value="Search" onclick=javascript:passv() >
			</div></li>
		</ol>
    </div>
  </div>
