...
class AdminEmailsRegisterReminder
	extends Page
{
	...
	protected function getPageData() {
		return array(
			'title'			=> 'Admin | Emails',
			'subtitle'		=> 'Registration Reminder Email',
	...
	public function __construct() {
		$this->subject = $this->config()->get('registerReminderSubject');
		$this->css = $this->config()->get('registerReminderCSS');
		$this->body = $this->config()->get('registerReminderBody');
	...
	public function updateAction() {
		...
		if(!$this->hasErrors()) {
			$this->config()->set('registerReminderSubject', $this->subject);
			$this->config()->set('registerReminderCSS', $this->css);
			$this->config()->set('registerReminderBody', $this->body);

			header('Location: adminEmailsRegisterReminderPreview.php');
		}
	}

}

new AdminEmailsRegisterReminder();
