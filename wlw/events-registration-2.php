...
$sqlprod = mysql_query("select * from t_provider where id='".$_GET['provider_id']."'");
	$rowprod = mysql_fetch_array($sqlprod);
	$_SESSION[WEBSITE_ALIAS]['language'] = $rowprod['language'];
...
if ( $form_action != '' ) {
	...
	unset($post_data['events_id']);
	unset($post_data['provider_id']);
	unset($post_data['titlecolor']);
	unset($post_data['bgcolor']);
	unset($post_data['titlebgcolor']);
	unset($post_data['tablegrid']);
...
switch ($form_action)
{
	case 'ADD':	
		...
		header ("Location: display-event-from-provider.php?provider_id=".$_REQUEST['provider_id']."&titlecolor=".$_REQUEST['titlecolor']."&bgcolor=".$_REQUEST['bgcolor']."&titlebgcolor=".$_REQUEST['titlebgcolor']."&tablegrid=".$_REQUEST['tablegrid']."");
		break;
...
<div id="header1" style="height:100px;padding-right:7px;background-color:transparent;width:500px;margin:0 auto;color:white;">
	
    <img src="images/header-logo.png" style="padding-top:30px;padding-left:10px;"/>
    
    
 </td>
</tr>
<tr>
<td style="background:url('images/bodybg.png') fixed;size:11px;font-size:11px;"> 
    
    <div id="content" style="border:1px solid #eeeeee;padding-bottom:20px;margin:0 auto;width:490px;size:11px;size:11px;font-size:11px;">
...
<?php if ( $mode == 'delete' ) { ?>
	<div id="system-message">
		<form action="events-registration-2.php" method="post" name="frm_<?php echo $page_name; ?>">
		
		
		
		<input type="hidden" name="provider_id" value="<?php echo $_REQUEST['provider_id']; ?>">
		<input type="hidden" name="titlecolor" value="<?php echo $_REQUEST['titlecolor']; ?>">
		<input type="hidden" name="bgcolor" value="<?php echo $_REQUEST['bgcolor']; ?>">
		<input type="hidden" name="titlebgcolor" value="<?php echo $_REQUEST['titlebgcolor']; ?>">
		<input type="hidden" name="tablegrid" value="<?php echo $_REQUEST['tablegrid']; ?>">
...
<div class="content-main wide80" style="size:11px;font-size:11px;size:11px;font-size:11px;width:470px;">
    ...
    <form action="events-registration-2.php" method="post" id="frm_<?php echo $page_name; ?>" enctype="multipart/form-data">
	<input type="hidden" name="provider_id" value="<?php echo $_REQUEST['provider_id']; ?>">
		<input type="hidden" name="titlecolor" value="<?php echo $_REQUEST['titlecolor']; ?>">
		<input type="hidden" name="bgcolor" value="<?php echo $_REQUEST['bgcolor']; ?>">
		<input type="hidden" name="titlebgcolor" value="<?php echo $_REQUEST['titlebgcolor']; ?>">
...
.ac_over {
	...
	<div style="display:none;float:right;width:380px;z-index:10000;position:absolute;margin-left:200px;margin-top:-18px;border:0px solid red;" id="textreminder"></div>
	  ...
	  ?>
		...
		?>">
            <?php if ( $is_editable_field ) { ?>
            &nbsp;&nbsp;<input class="button" name="btnCancel" onclick="history.go(-1);" type="button" value="<?php
		$sqlfield = mysql_query("select * from t_field_names where id=271");
		$rowfield = mysql_fetch_array($sqlfield);
		if ($_SESSION[WEBSITE_ALIAS]['language'] ==1){
			echo $rowfield['fieldname_de'];
