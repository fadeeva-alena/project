...
$sqlvat = mysql_query("select * from t_vat");
...
if("SUCCESS" == strtoupper($httpParsedResponseAr["ACK"]) || "SUCCESSWITHWARNING" == strtoupper($httpParsedResponseAr["ACK"])) {
  ...
  if ($_REQUEST['access_num'] == 1){
	...
	if (mysql_num_rows($sqlcheckfirst) > 0){
		...
		$billingtop = "<div style=float:left;><img src='http://manimano.ch/wlw/images/header-logo.png'><br />DSC GmbH, Längistr. 12, 8132 Egg</div>";

		$useraddress = "";
		if ($row->company !=""){
			$useraddress .= fixEncodingx($row->company) . " ";
		}
		if ($row->adress1 !=""){
			$useraddress .= fixEncodingx($row->adress1) . " ";
		}
		if ($row->adress2 !=""){
			$useraddress .= fixEncodingx($row->adress2) . " ";
		}
		if ($row->location !=""){
		...
		$bodyemail = str_replace("<firstname> <lastname>,","<div style='float:right;margin-top:10px;text-align:right;'><b>".genderfields($gender) . " " . $row->firstname ." " . $row->lastname."</b><br />".$useraddress."</div><br style='clear:both;'><hr>",$bodyemail);
		//in between: title amount option priceeach pricetotal
		$priceinfo = "(10.00 * ".$addmonth .") + ".$vat." = ". $amount . "<br />VAT:".$vat."<br/><br />";
		
		$bodyemail = str_replace("<our_footer>",$priceinfo. "". translatefields(649),$bodyemail);
		$bodyemail = str_replace(",,","",$bodyemail);
		$body = $body . "" . $bodyemail . "</div>";
	...
	if (mysql_num_rows($sqlcheckfirst) > 0){
		...
		$bodyemail = str_replace("<firstname> <lastname>,","<div style='float:right;margin-top:10px;text-align:right;'><b>".genderfields($gender) . " " . $row->firstname ." " . $row->lastname."</b><br />".$useraddress."</div><br style='clear:both;'><hr>",$bodyemail);
		//in between: title amount option priceeach pricetotal
		//$priceinfo = "10.00 * ".$addmonth ." = ". $amount . "<br /><br />";
		$priceinfo = "(10.00 * ".$addmonth .") + ".$vat." = ". $amount . "<br />VAT:".$vat."<br/><br />";
	...
	$body = "<div style='font-family:tahoma;font-size:13px;size:13px;'>";
		...
		if ($_REQUEST['access_num'] == 4){
			//$priceinfo = "20.00 * ".$addmonth ." = ". $amount . "<br /><br />";
			$priceinfo = "(20.00 * ".$addmonth .") + ".$vat." = ". $amount . "<br />VAT:".$vat."<br/><br />";
		}else{
			//$priceinfo = "10.00 * ".$addmonth ." = ". $amount . "<br /><br />";
			$priceinfo = "(10.00 * ".$addmonth .") + ".$vat." = ". $amount . "<br />VAT:".$vat."<br/><br />";
		}
	...
	//header("location: index.php?option=access&tobook=".$_REQUEST['access_num']."&error=true");
...
?>
    <div id="system-message">
    <div class="info">
     <div class="message"><?php echo translatefields($message_code);?></div>
     </div>
     </div>
     <?php } ?>	 
<?php
	 ...
	 if (num == 2 || num == 4){
	  ...
	  <?php if ($seminar_with_reservation == 0){ ?>
		if (!confirm("<?php echo translatefields2(798);?>")) {
			...
			//document.location.href ='index.php?option=access&num_plan='+num+'&task=booked';
			$('#payform').show();
		  $('#access_num').val(num);
...
$(document).ready(function() {
	...
	<form name="payform" action="index.php?option=access&task=booked" method="post" id="payform" style="display:none;">
		...
		<tbody>
			...
			<tr class="row1">
				<td><b><?php echo translatefields(804);?></b></td><td><input name="months_plan" type="text" value="2" id="months_plan"/></td>	
			</tr>
			<tr>
			...
			<tr class="row1">
				<td><b><?php echo translatefields(806);?></b></td><td><input name="cardnumber" type="text" value="4436701199871582"></td>	
			</tr>
			<tr>
				<td><b><?php echo translatefields(807);?></b> <span style=" font:7pt arial; color:gray;">[ mm / yyyy ]</span></td><td>
				<select name="cardmonth" style="width:45px;">
				...
				&nbsp;
				<input type="text" name="cardyear" class="sel" style=" width:50px;" value="2017"></td>	
			</tr>
			<tr class="row1">
				<td><div style="float:left;width:130px;"><b><?php echo translatefields(8028);?>Sicherheitsnummer (CCV, hinten)</b></div><div style="float:right;"><a href="images/cvv.jpeg" id="cvv"><img border=0 src="images/cvv.jpeg"  width="30"></div></td><td><input name="cardcvv" type="text" value=""></a>
				<script>
					$('#cvv').lightBox({
			...
			<tr>
				<td></td><td><input type="submit" value="<?php echo translatefields(809);?>" style="border:1px outset gray; width:100px;"></td>
			</tr>
		</tbody>
		</table>
	</form>
