...
http://localhost/json/components/display-event.php?events_id=1183&titlecolor=5EFF79&bgcolor=E9E9E9&titlebgcolor=CCCCCC&tablegrid=CCCCCC
...
// Retrieve record
	...
	<div class="content-main" style="width:500px;">
		...
		<form action="<?php echo INDEX_PAGE . $page_option ?>" method="post" id="frm_<?php echo $page_name?>"  enctype="multipart/form-data">
			...
			<input type="hidden" name="search_keyword" id="search_keyword" value="<?php echo $_GET['search_keyword'];?>" />
				...
				<table class="form-table" style="border:1px solid #<?php echo $_GET['tablegrid'];?>">            	
					<tr>
						<td class="key" style="background-color:<?php if ($_GET['titlebgcolor'] == "") { echo "#EFEFEF"; } else { echo "#" . $_GET['titlebgcolor'];} ?>;"><label for="title">
		<?php
		...
		<div style="display:none;float:right;width:220px;z-index:10000;position:absolute;margin-left:200px;margin-top:-18px;border:0px solid red;" id="textlastname"></div>
						...
						<?php } else { ?>
						<td width="290px"><?php echo fixEncoding($title)?></td>
						<?php } ?>                                                                                                    
					</tr>
...
if (date('Y-m-d') >= $size_row['events_start_date'] and date('Y-m-d') <= $size_row['events_end_date']){	
	...
	echo "<div style='float:left;margin-left:14px;width:65px;border:0px solid red;'>".date('d.m.Y',strtotime($size_row['events_start_date']))."</div>";
	  if ($_SESSION[WEBSITE_ALIAS]['admin_id'] ==""){
		...
		if ($size_row['events_start_date'] != $size_row['events_end_date']){
			echo "<div style='float:left;margin-left:4px;width:65px;border:0px solid red;'>".date('d.m.Y',strtotime($size_row['events_end_date']))."</div></div><div style='float:left;width:60px;'><a alt='".$downloadevent."' title='".$downloadevent."' href='components/download-ical.php?id=".$_GET['events_id']."&date_id=".$size_row['id']."'><img src='images/icon-downloads.png' width='16px' height='16px' border='0'/></a> <a alt='".$emailevent."' title='".$emailevent."' style=cursor:pointer; onclick=emailevent(1,$rowevent[eid],$size_row[id])><img src='images/icon-send-email.png' width='16px' height='16px' border='0'/></a>";
