...
?>
	...
	?>
		...
		*/
		...
		if ($_GET['providername'] != ""){
			...
			$providername_session = $_SESSION['events_providername2'];
		}			
	...
	if ($_SESSION[WEBSITE_ALIAS]['language'] ==1){$requestor =  $rowfield['fieldname_de'];
	 ...
	 {display: '<?php echo $requestor;?>', name : '', width : 120, sortable : true, align: 'left'},
     {display: '<?php echo $name;?>', name : '', width : 150, sortable : true, align: 'left'},
    ...
    // {display: '<?php echo $address2;?>', name : 'loc_adress2', width : 93, sortable : true, align: 'center'},
	 {display: '<?php echo $contact;?>', name : '', width : 140, sortable : true, align: 'left'},
     ...
     return false;
		...
		{
		var numSelected = $("input.locationsid:checked").length;
			if (numSelected > 0){
			
				var allVals = [];
     			$('input.locationsid:checked').each(function() {
				...
				$.ajax 
					({
						type: "POST",
						dataType: "json",
						url: "<?php echo PATH_COMPONENTS?>process-handover-locations-request-approve.php?locationsid="+ allVals,
						data: "locationsid="+ allVals,
						success: function(data) {
     			...
     			$('input.locationsid:checked').each(function() {
       				allVals.push($(this).val());
								...
								({
									type: "POST",
									dataType: "json",
									url: "<?php echo PATH_COMPONENTS?>process-handover-locations-request-disapprove.php?locationsid="+ allVals,
									data: "reason="+ note,
									success: function(data) {
											<?php
...
?>alert("<?php echo $successfull;?>");
     ...
     else if (com=='<?php echo $selectall;?>')
     {
         ...
         $('.locationsid').attr('checked',true);
     }				
     else if (com=='<?php echo $deselectall;?>')
     {
         //$('.bDiv tbody tr',grid).removeClass('trSelected');
         $('.locationsid').attr('checked',false);
     }				
     }
