...
session_start();
	...
	<form id="frm_customsearch">
		...
		$rowfield = mysql_fetch_array($sqlfield);
		if ($_SESSION[WEBSITE_ALIAS]['language'] ==1){
			$id = $rowfield['fieldname_de'];
		}elseif ($_SESSION[WEBSITE_ALIAS]['language'] ==2){
			$id =$rowfield['fieldname_eng'];
		}elseif ($_SESSION[WEBSITE_ALIAS]['language'] ==3){
			$id =$rowfield['fieldname_fr'];
		}elseif ($_SESSION[WEBSITE_ALIAS]['language'] ==4){
			$id =$rowfield['fieldname_it'];
		}
...
<link href="plugins/jquery/fancybox/jquery.fancybox-1.3.1.css" rel="stylesheet" type="text/css" /> 
     ...
     ({
     url: '<?php echo PATH_COMPONENTS;?>translations-list.php?optionlist=<?php echo $_SESSION['optionlist'];?>',
     dataType: 'json',
     colModel : 
     [			
     {display: '<?php echo $action;?>', width : <?php echo $config['grid']['3action'];?>, align: 'center'},
	 {display: '<?php echo $id;?>', name : 'id', width : 40, sortable : true, align: 'right'},
     {display: '<?php echo $fieldname;?>', name : 'fieldname', width : 140, sortable : true, align: 'left'},
     {display: '<?php echo $fieldname_de;?>', name : 'fieldname_de', width : 155, sortable : true, align: 'left'},
     {display: '<?php echo $fieldname_en;?>', name : 'fieldname_eng', width : 155, sortable : true, align: 'left'},
	 {display: '<?php echo $fieldname_fr;?>', name : 'fieldname_fr', width : 155, sortable : true, align: 'left'},
     {display: '<?php echo $fieldname_it;?>', name : 'fieldname_it', width : 155, sortable : true, align: 'left'}
     ],
