...
if ($_REQUEST['task'] == "delete-img"){
		mysql_query("update t_leader set image_path='' where id='$id'");
		header("Location: ".INDEX_PAGE."leaders-m&providername=".$_REQUEST['providername']."&search_keyword=".$_REQUEST['search_keyword']."&mode=edit&id=".$id."&delimgsuccess=true#deleteimg");
		exit();
	}
			...
			<?php
				...
				if ($mode == "edit"){
					if ($design_photo_img != ""){
						...
						?>
						<input type="button" value="<?php echo fixEncoding(translatefields(759));?>" onclick="document.location.href='index.php?option=leaders-m&providername=<?php echo $_REQUEST['providername'];?>&search_keyword=<?php echo $_REQUEST['search_keyword'];?>&mode=edit&id=<?php echo $id;?>&task=delete-img'" class="button"/><br /><br />
						<?php
					}else{
