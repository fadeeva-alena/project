...
while ($row = mysql_fetch_array($result))
{
	...
	$feedbackevent = "<div class=widthfix>".$row['feedback_events'] . " <br /><a style=cursor:pointer; onclick=deny(".$row['fid'].",1,0)>".$denylabel."</a> | <a style=cursor:pointer; onclick=accept(".$row['fid'].",1,1)>".$acceptlabel."</a></div>";
	}elseif ($row['feedback_events_accepted'] == 1){
	$feedbackevent = "<div class=widthfix>".$row['feedback_events']."</div>";
	}else{
	$feedbackevent = "<div class=widthfix>".$row['feedback_events']."<br /><span style=color:red;font-color:red;>".$denied."</span></div>";
	}
	$json .= ",'".$feedbackevent."'";

	if ($row['feedback_locations_accepted'] == 0){
	$feedbacklocation = "<div class=widthfix>".$row['feedback_locations'] . " <br /><a style=cursor:pointer; onclick=deny(".$row['fid'].",2,0)>".$denylabel."</a> | <a style=cursor:pointer; onclick=accept(".$row['fid'].",2,1)>".$acceptlabel."</a></div>";
	}elseif ($row['feedback_locations_accepted'] == 1){
	$feedbacklocation = "<div class=widthfix>".$row['feedback_locations']."</div>";
	}else{
	$feedbacklocation = "<div class=widthfix>".$row['feedback_locations']."<br /><span style=color:red;font-color:red;>".$denied."</span></div>";
	}
	$json .= ",'".$feedbacklocation."'";
