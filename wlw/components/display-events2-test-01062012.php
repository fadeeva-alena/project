...
if ($quality ==1){
	...
	echo "<div style='float:left;margin-left:14px;width:65px;border:0px solid red;'>".date('d.m.Y',strtotime($size_row['events_start_date']))."</div>";
	  if ($_SESSION[WEBSITE_ALIAS]['admin_id'] ==""){
		...
		if ($size_row['events_start_date'] != $size_row['events_end_date']){
			echo "<div style='float:left;margin-left:4px;width:65px;border:0px solid red;'>".date('d.m.Y',strtotime($size_row['events_end_date']))."</div></div><div style='float:left;width:60px;'><a href='components/download-ical.php?id=".$rowevent['eid']."&date_id=".$size_row['id']."'><img src='images/icon-downloads.png' width='16px' height='16px' border='0'/></a> <a style=cursor:pointer; onclick=emailevent(1,$rowevent[eid],$size_row[id])><img src='images/icon-send-email.png' width='16px' height='16px' border='0'/></a>";
		if ($rowevent['active_for_reservation'] == 1){	
			$checkreservation = mysql_query("select * from t_reservations where date_id='".$size_row['id']."' and provider_id='".$_SESSION[WEBSITE_ALIAS]['admin_id']."' and (status=1 or status=0)");
			if (mysql_num_rows($checkreservation) > 0){
				...
				$count = 0;
				$up = 0;
				$checkupordown = mysql_query("select * from t_reservations where date_id='".$size_row['id']."' and (status=1)");;
				while ($result= mysql_fetch_array($checkupordown)){
					$count++;
					if (($count <= $rowevent['max_number']) and ($result['date_id'] == $size_row['id']) and ($result['provider_id'] == $provider_id)){
						$up++;
					}
				}
				
					if ($up > 0){
						echo "<img src='images/hands-up.png' width='16px' height='16px' border='0' /> ";
					}else{
						echo "<img src='images/hands-down.png' width='16px' height='16px' border='0' /> ";
					}
					echo "<a style=cursor:pointer; onclick='datereserveagain($rowevent[eid],$size_row[id],$provider_id)'><img src='images/reservation.png' width='16px' height='16px' border='0'/></a> ";
				...
				$provider_id = $_SESSION[WEBSITE_ALIAS]['admin_id'];
				echo "<div id=reservation_div_".$size_row['id'].">";
				echo "<a style=cursor:pointer; onclick='datereserve($rowevent[eid],$size_row[id],$provider_id)'><img src='images/reservation.png' width='16px' height='16px' border='0'/></a>";
				echo "</div>";
			}
		}
			echo "</div><div id=mailprocessing".$size_row[id]." class=mailprocessing style=float:left;></div><br style=clear:both;>";
		}else{
			echo "<div style='float:left;margin-left:4px;width:65px;border:0px solid red;'>&nbsp;</div></div><div style='float:left;width:60px;'><a style=cursor:pointer; href='components/download-ical.php?id=".$rowevent['eid']."&date_id=".$size_row['id']."'><img src='images/icon-downloads.png' width='16px' height='16px' border='0'/></a> <a style=cursor:pointer; onclick=emailevent(1,$rowevent[eid],$size_row[id])><img src='images/icon-send-email.png' width='16px' height='16px' border='0'/></a>";
		if ($rowevent['active_for_reservation'] == 1){	
			$checkreservation = mysql_query("select * from t_reservations where date_id='".$size_row['id']."' and provider_id='".$_SESSION[WEBSITE_ALIAS]['admin_id']."' and (status=1 or status=0)");
			if (mysql_num_rows($checkreservation) > 0){
				$provider_id = $_SESSION[WEBSITE_ALIAS]['admin_id'];
				...
				while ($result= mysql_fetch_array($checkupordown)){
					...
					if ($up > 0){
						...
						$rowfield = mysql_fetch_array($sqlfield);
						if ($_SESSION[WEBSITE_ALIAS]['language'] ==1){
							$handsup = $rowfield['fieldname_de'];
						}elseif ($_SESSION[WEBSITE_ALIAS]['language'] ==2){
							$handsup = $rowfield['fieldname_eng'];
						}elseif ($_SESSION[WEBSITE_ALIAS]['language'] ==3){
							$handsup = $rowfield['fieldname_fr'];
						}elseif ($_SESSION[WEBSITE_ALIAS]['language'] ==4){
							$handsup = $rowfield['fieldname_it'];
						}
						echo "<img src='images/hands-up.png' width='16px' alt='".$handsup."' title='".$handsup."' height='16px' border='0' /> ";
					}else{
						$sqlfield = mysql_query("select * from t_field_names where id=412");
						$rowfield = mysql_fetch_array($sqlfield);
						if ($_SESSION[WEBSITE_ALIAS]['language'] ==1){
							$handsdown = $rowfield['fieldname_de'];
						}elseif ($_SESSION[WEBSITE_ALIAS]['language'] ==2){
							$handsdown = $rowfield['fieldname_eng'];
						}elseif ($_SESSION[WEBSITE_ALIAS]['language'] ==3){
							$handsdown = $rowfield['fieldname_fr'];
						}elseif ($_SESSION[WEBSITE_ALIAS]['language'] ==4){
							$handsdown = $rowfield['fieldname_it'];
						}
						echo "<img src='images/hands-down.png' alt='".$handsdown."' title='".$handsdown."' width='16px' height='16px' border='0' /> ";
					}
					$sqlfield = mysql_query("select * from t_field_names where id=410");
						$rowfield = mysql_fetch_array($sqlfield);
						if ($_SESSION[WEBSITE_ALIAS]['language'] ==1){
							$cancelreservation = $rowfield['fieldname_de'];
						}elseif ($_SESSION[WEBSITE_ALIAS]['language'] ==2){
							$cancelreservation = $rowfield['fieldname_eng'];
						}elseif ($_SESSION[WEBSITE_ALIAS]['language'] ==3){
							$cancelreservation = $rowfield['fieldname_fr'];
						}elseif ($_SESSION[WEBSITE_ALIAS]['language'] ==4){
							$cancelreservation = $rowfield['fieldname_it'];
						}
				echo "<a alt='".$cancelreservation."' title='".$cancelreservation."' style=cursor:pointer; onclick='datecancel($rowevent[eid],$size_row[id],$provider_id)'><img src='images/cancel.png' width='16px' height='16px' border='0' /></a>";
				echo "</div>";
			}else{
				$provider_id = $_SESSION[WEBSITE_ALIAS]['admin_id'];
				echo "<div id=reservation_div_".$size_row['id'].">";
				...
				echo "<a alt='".$reservationbutton."' title='".$reservationbutton."' style=cursor:pointer; onclick='datereserve($rowevent[eid],$size_row[id],$provider_id)'><img src='images/reservation.png' width='16px' height='16px' border='0'/></a>";
				echo "</div>";
			}
		}
			echo "</div><div id=mailprocessing".$size_row[id]." class=mailprocessing style=float:left;></div><br style=clear:both;>";
