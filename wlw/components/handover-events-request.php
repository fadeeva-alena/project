...
<script type="text/javascript">
	...
	({
		...
		colModel : 
			...
			{display: '<?php echo $requestor;?>', name : '', width : 130, sortable : true, align: 'left'},
			{display: '<?php echo $title;?>', name : 'title', width : 200, sortable : true, align: 'left'},
	...
	function buttonAction(com,grid)
	{
		...
		{
		var numSelected = $("input.eventsid:checked").length;
			if (numSelected > 0){
			
				var allVals = [];
     			$('input.eventsid:checked').each(function() {
				...
				$.ajax 
					({
						type: "POST",
						dataType: "json",
						url: "<?php echo PATH_COMPONENTS?>process-handover-events-request-approve.php?eventsid="+ allVals,
						data: "eventsid="+ allVals,
						success: function(data) {
							<?php
     			...
     			$('input.eventsid:checked').each(function() {
       				allVals.push($(this).val());
								...
								({
									type: "POST",
									dataType: "json",
									url: "<?php echo PATH_COMPONENTS?>process-handover-events-request-disapprove.php?eventsid="+ allVals,
									data: "reason="+ note,
									success: function(data) {
											<?php
