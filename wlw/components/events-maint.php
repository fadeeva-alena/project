...
$lastDayOfTheMonth   = date('Y-m-d',mktime(0,0,0,$currentMonthNumeric + 1,0,$currentFullYear));
   ...
   echo $lastDayOfTheMonth .PHP_EOL;*/
	...
	if ($_REQUEST['task'] == "delete-img"){
		mysql_query("update t_event set eve_image_path='' where id='$id'");
		header("Location: ".INDEX_PAGE."events-m&providername=".$_REQUEST['providername']."&search_keyword=".$_REQUEST['search_keyword']."&mode=edit&id=".$id."&delimgsuccess=true#deleteimg");
		exit();
	}
...
//add_leader_option
					...
					for ($dx =0;$dx <$daysindaily;$dx++){
						...
						if (($timeStamp1 <= strtotime($_POST['planning_end_date_' . $count])) and ($timeStamp <=strtotime($_POST['planning_end_date_' . $count]))){
							if ($date2 == $date1){
								if ($date_start != ""){
									mysql_query("insert into t_dates values ('0','$id','$startenddateweek','$startenddateweek')");
									//echo "<br/>--------insert into t_dates values ('0','$id','$startenddateweek','$startenddateweek')";
								}
							}else{
								if ($date_start != ""){
									$timeStampx = "";
									$timeStampx = strtotime($date_start);
									$timeStampx += 24 * 60 * 60 * ($dx+($daycomparex));
									$enddatedaily = date('Y-m-d',$timeStampx);	
									mysql_query("insert into t_dates values ('0','$id','$startenddateweek','$enddatedaily')");
									//echo "<br/>+++++insert into t_dates values ('0','$id','$startenddateweek','$enddatedaily')";
								}
							}
						}
					}
	...
	for ($i = 1;$i<=$duration;$i++){
		...
		//lastevent
		$sqlchecklast = mysql_query("select * from tbl_lastevent where admin_id='".$_SESSION[WEBSITE_ALIAS]['admin_id']."'");
		if (mysql_num_rows($sqlchecklast) > 0){
			$rowchecklast = mysql_fetch_array($sqlchecklast);
			mysql_query("update tbl_lastevent set lastevent_id='$id' where id='".$rowchecklast['id']."'");
		}else{
			$saveuserid =$_SESSION[WEBSITE_ALIAS]['admin_id'];
			mysql_query("insert into tbl_lastevent values ('0','$id','$saveuserid')");
		}
	...
	if ($mode == "add" and $_GET['type']==1){
		...
		if (mysql_num_rows($sqlchecklast) > 0){
			$rowchecklast = mysql_fetch_array($sqlchecklast);
			$ids = $rowchecklast['lastevent_id'];
			$record = $sql_helper->cget_row(DB_TABLE_PREFIX."event", "id = '$ids'");
		}else{
			$sqllastentry = mysql_query("select * from t_event where provider='".$_SESSION[WEBSITE_ALIAS]['admin_id']."'
					order by id desc");
			$rowlastentry = mysql_fetch_array($sqllastentry);
...
$().ready(function() {
	...
	var locsuggest = [
		...
		?>
		{ locid: "<?php echo utf8_encode($locid);?>", location: "<div style=float:left;width:35;margin:right:10px;><?php if ($result['loc_image_path']!= ""){?><img width=30px src=uploads/<?php echo $result['loc_image_path'];?>><?php } ?></div><div style=float:left;> <?php $locsuggest = str_replace('"','',trim(utf8_encode($locsuggest)));echo str_replace(';','',trim(utf8_encode($locsuggest)));?></div>", locationvalue: "<?php echo trim(utf8_decode($locsuggest));?>" }
		<?php
			}else{
		?>
		{ locid: "<?php echo utf8_encode($locid);?>", location: "<div style=float:left;width:35;margin:right:10px;><?php if ($result['loc_image_path']!= ""){?><img width=30px src=uploads/<?php echo $result['loc_image_path'];?>><?php } ?></div><div style=float:left;> <?php $locsuggest = str_replace('"','',trim(utf8_encode($locsuggest)));echo str_replace(';','',trim(utf8_encode($locsuggest)));?></div>" , locationvalue: "<?php echo trim(utf8_decode($locsuggest));?>"},
	...
	var leadersuggest = [
		...
		{ leaderid: "<?php echo utf8_encode($leaderid);?>", leader: "<div style=float:left;width:35;margin:right:10px;><?php if ($result['image_path']!= ""){?><img width=30px src=uploads/<?php echo $result['image_path'];?>><?php } ?>&nbsp;</div><div style=float:left;> <?php $leadersuggest = str_replace('"','',trim(utf8_encode($leadersuggest)));echo str_replace(';','',trim(utf8_encode($leadersuggest)));?></div>", leadervalue: "<?php $leadersuggest = str_replace('"','',trim(utf8_encode($leadersuggest)));echo str_replace(';','',trim(utf8_encode($leadersuggest)));?>" }
		<?php
			}else{
		?>
		{ leaderid: "<?php echo utf8_encode($leaderid);?>", leader: "<div style=float:left;width:35;margin:right:10px;><?php if ($result['image_path']!= ""){?><img width=30px src=uploads/<?php echo $result['image_path'];?>><?php } ?>&nbsp;</div><div style=float:left;> <?php $leadersuggest = str_replace('"','',trim(utf8_encode($leadersuggest)));echo str_replace(';','',trim(utf8_encode($leadersuggest)));?></div>", leadervalue: "<?php $leadersuggest = str_replace('"','',trim(utf8_encode($leadersuggest)));echo str_replace(';','',trim(utf8_encode($leadersuggest)));?>" },
	...
	var leader2suggest = [
		...
		?>
		{ leader2id: "<?php echo utf8_encode($leader2id);?>", leader2: "<div style=float:left;width:35;margin:right:10px;><?php if ($result['image_path']!= ""){?><img width=30px src=uploads/<?php echo $result['image_path'];?>><?php } ?>&nbsp;</div><div style=float:left;> <?php $leader2suggest = str_replace('"','',trim(utf8_encode($leader2suggest)));echo str_replace(';','',trim(utf8_encode($leader2suggest)));?></div>", leader2value: "<?php $leader2suggest = str_replace('"','',trim(utf8_encode($leader2suggest)));echo str_replace(';','',trim(utf8_encode($leader2suggest)));?>" }
		<?php
			}else{
		?>
		{ leader2id: "<?php echo utf8_encode($leader2id);?>", leader2: "<div style=float:left;width:35;margin:right:10px;><?php if ($result['image_path']!= ""){?><img width=30px src=uploads/<?php echo $result['image_path'];?>><?php } ?>&nbsp;</div><div style=float:left;> <?php $leader2suggest = str_replace('"','',trim(utf8_encode($leader2suggest)));echo str_replace(';','',trim(utf8_encode($leader2suggest)));?></div>", leader2value: "<?php $leader2suggest = str_replace('"','',trim(utf8_encode($leader2suggest)));echo str_replace(';','',trim(utf8_encode($leader2suggest)));?>" },
...
.ac_over {
	...
	<tr>
		...
		?> <?php echo $req_fld?></label></td>
				...
				<input type="hidden" id="locid" name="locationid" value="<?php if ($mode == "add") { echo $record->location;} else {echo $record->location;}?>" /><br /><?php
		...
		?><?php echo $req_fld;?></label></td>
			 <?php if ( $is_editable_field ) { ?>
				...
				<input type="hidden" id="leader1id" name="leader1id" value="<?php if ($mode == "add") { echo $rowleader['id'];} else {echo $record->leader;}?>" />
...
<script>
		...
		?></label></td>
			 <?php if ( $is_editable_field ) { ?>
				...
				<input type="hidden" id="leader2id" name="leader2id" value="<?php if ($mode == "add") { echo $leader2;} else {echo $record->leader2;}?>" />
		...
		?>
			...
			<?php if ( isset($_GET['delimgsuccess']) && $_GET['delimgsuccess'] != '' ) { ?>
			<div id="system-message">
				<div class="info">
					<div class="message"><?php echo fixEncoding(translatefields(760));?></div>
				</div>
			</div>
			<?php } ?>
			<?php
				...
				if ($mode == "edit"){
					if ($design_photo_img != ""){
						echo $design_photo_img . "<br />";
						?>
						<input type="button" value="<?php echo fixEncoding(translatefields(759));?>" onclick="document.location.href='index.php?option=events-m&providername=<?php echo $_REQUEST['providername'];?>&search_keyword=<?php echo $_REQUEST['search_keyword'];?>&mode=edit&id=<?php echo $id;?>&task=delete-img'" class="button"/><br /><br />
						<?php
					}else{
						echo $design_photo_img . "<p>";
					}
				}
			?>
