...
$_SESSION['sessionreservationstartdate'] =strtotime($startreservationdate);
$_SESSION['sessionreservationenddate'] = strtotime($endreservationdate);

$weekstodisplay = "&start=".$_SESSION['sessionreservationstartdate']."&end=".$_SESSION['sessionreservationenddate'];

$sqlreservations = mysql_query("select * from t_provider_events pe inner join t_provider_reservation pr on pe.provider_event_id=pr.provider_event_id inner join t_currency c on c.id=pe.provider_event_price_currency where reservation_status=1 and pe.provider_id='".$_SESSION['ownerproviderid']."' and reservation_start_date>='$startreservationdate' and reservation_end_date<='$endreservationdate'");
...
$sqlblocked = mysql_query("select * from t_provider_reservation where owner_provider_id='".$_SESSION['ownerproviderid']."' and provider_event_id=0 and (reservation_status=3 or reservation_status=4) and reservation_start_date>='$startreservationdate' and reservation_end_date<='$endreservationdate'");
	...
	$sqllunch = mysql_query("select * from t_provider_reservation where owner_provider_id='".$_SESSION['ownerproviderid']."' and provider_event_id=0 and (reservation_status=5) and reservation_start_date>='$startreservationdate' and reservation_end_date<='$endreservationdate'");
    ...
    while ($startloopdate <= $endloopdate) { 
		...
		if ($weekrange >= $lunch_day_start and $weekrange <= $lunch_day_end){
			...
			$checklunchreservation = mysql_query("select * from t_provider_reservation where reservation_start_date like '%$checkifnot%' and reservation_status=5 and owner_provider_id='".$_SESSION['ownerproviderid']."'");
