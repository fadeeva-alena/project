...
//////////////////////////////////for the filter sorting
	if ($_GET['location']!="locname"){
		...
		$rows = "";
		 ...
		 where (x.events_start_date <= '{$datetocheck}' and x.events_end_date >= '{$datetocheck}') or ((l.loc_name LIKE '%$locationx%' OR l.loc_detail LIKE '%$locationx%' OR l.loc_adress1 LIKE '%$locationx%' OR l.loc_adress2 LIKE '%$locationx%' OR l.loc_zip LIKE '%$locationx%' OR l.loc_loc LIKE '%$locationx%')) $getquality group by e.id";
		 }else{
			$keyword = $_GET['keyword'];
			$sqlqry = "SELECT * FROM t_event e 
         ...
         $distance = number_format(distance($eventlat, $eventlong,$currentlat, $currentlong, 'K'),2);
...
if ($_GET['keyword'] == ""){
$sql = "select *,e.id as eid from t_event e inner join t_dates d
								on e.id=d.events_id 
								inner join t_location l
								on e.location =l.id
								where $where (events_start_date <= '{$datetocheck}' and events_end_date >= '{$datetocheck}') $getquality GROUP BY e.id $order";
}else{
$keyword = $_GET['keyword'];
$sql = "SELECT * FROM t_event e 
								INNER JOIN t_dates d
								ON e.id=d.events_id
								LEFT JOIN t_event_kind ek
								ON e.kind=ek.id
