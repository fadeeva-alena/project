				...
				$testtoexport .='<tr>
					...
					if ($rowfields['reservation_status'] == 0){
						...
						if (($results <= $size_row['cancellation_day1'])){
							...
							$subtract1= date("Y-m-d",$subtract1);
							
							$subtract5 = strtotime(date("Y-m-d", strtotime($startdate)) . " -".$size_row['cancellation_day2']." day");
							$subtract5= date("Y-m-d",$subtract5);
							
							if ($subtract1 > $today and $subtract5 > $today){
								//echo "cancel with no fee";
								//$feeamount = $size_row['cancellation_fee2'];
								...
								if ($size_row['cancellation_day1'] < $size_row['cancellation_day2']){
									if (($results <= $size_row['cancellation_day1'])){
										$feeamount = $size_row['cancellation_fee1'];
									}elseif(($results <= $size_row['cancellation_day2']))
									{
										$feeamount = $size_row['cancellation_fee2'];
									}
								}elseif($result>$size_row['cancellation_day1'] AND $result>=$size_row['cancellation_day2'])
								{
