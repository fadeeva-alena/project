	...
	switch ($form_action)
	{
			...
			if ($_POST['dateoption_' . $count] == "daily"){
				if ($_POST['fixorduration_' . $count] == "duration"){
					...
					for ($dx =0;$dx <$daysindaily;$dx++){
						...
						$timeStamp += 24 * 60 * 60 * $dx;
						$startenddateweek = date('Y-m-d',$timeStamp);
						if ($timeStamp <= strtotime($_POST['planning_end_date_' . $count])){
							mysql_query("insert into t_dates values ('0','$id','$startenddateweek','$startenddateweek')");
						}
					}
				}
				...
				if ($_POST['fixorduration_' . $count] == "duration"){
					...
					$daysinweeks = $_POST['duration_'. $count] * 7;
					...
					for ($dx =1;$dx <$daysinweeks;$dx++){
						$timeStamp = "";
						$timeStamp = strtotime($date_start);
						$timeStamp += 24 * 60 * 60 * $dx;
						echo $day_name_compare = date("N",$timeStamp);
						echo "-";
						echo $date2 = date('N',strtotime($_POST['end_date_' . $count]));
						echo "-";
						echo $date1 = date('N',strtotime($_POST['start_date_' . $count]));
						echo "-";
						echo "<br />";
						if ($day_name_compare == $date2 or $day_name_compare == $date1){
							if ($day_name_compare == $date2){
					...
					$date2c = date('m/d/Y',strtotime($_POST['planning_end_date_' . $count]));
					$date1c = date('m/d/Y',strtotime($_POST['start_date_' . $count]));
					echo $daysinweeks = dateDiff("/", $date2c, $date1c);	
					//$daysinweeks = $_POST['duration_'. $count] * 7;
					$datestartweek = date('Y-m-d',strtotime($date_start));
					mysql_query("insert into t_dates values ('0','$id','$datestartweek','$datestartweek')");
					...
					for ($dx =1;$dx <$daysinweeks;$dx++){
						...
						echo "-";
						echo "<br />";
						if ($day_name_compare <= $date2 and $day_name_compare >= $date1){
							$startenddateweek = date('Y-m-d',$timeStamp);
							if ($timeStamp <= strtotime($_POST['planning_end_date_' . $count])){
								mysql_query("insert into t_dates values ('0','$id','$startenddateweek','$startenddateweek')");
							}
				...
				if ($_POST['fixorduration_' . $count] == "duration"){
					$daysinweeks = $_POST['duration_'. $count] * 31;
					//$datestartweek = date('Y-m-d',strtotime($date_start));
					//mysql_query("insert into t_dates values ('0','$id','$datestartweek','$datestartweek')");
					$datestartweek = date('Y-m-d',strtotime($date_start));
					$dateendweek = date('Y-m-d',strtotime($_POST['end_date_'.$count]));
					...
					for ($dx =1;$dx <$daysinweeks;$dx++){
						...
						$timeStamp = strtotime($date_start);
						$timeStamp += 24 * 60 * 60 * $dx;
						echo $day_name_compare = date("d",$timeStamp);
						echo "--";
						echo $date2 = date('d',strtotime($_POST['end_date_' . $count]));
						echo "-";
						echo $date1 = date('d',strtotime($_POST['start_date_' . $count]));
						echo "-";
						echo "<br />";
						/*if ($day_name_compare <= $date2 or $day_name_compare >= $date1){
							$startenddateweek = date('Y-m-d',$timeStamp);
						...
						if ($day_name_compare == $date2 or $day_name_compare == $date1){
							if ($day_name_compare == $date2){
								if ($date2 == $date1){
									...
									echo $enddate1 = date('Y-m-d',$timeStamp);
								}
							}else{
								echo $startdate1 = date('Y-m-d',$timeStamp);
							}
							
							if ($enddate1 != "" and $startdate1 != ""){
								mysql_query("insert into t_dates values ('0','$id','$startdate1','$enddate1')");
								
								echo "insert into t_dates values ('0','$id','$startdate1','$enddate1')";
								$startdate1 = "";
								$enddate1 = "";
							}
						}
		...
		header("Location: ".INDEX_PAGE."events3&a=add&success=".$result);
		break;
		...
		//header("Location: ".INDEX_PAGE."events3&a=edit&success=".$result);
		
		header("Location: ".INDEX_PAGE."events3&providername=".$_POST['providername']."&search_keyword=".$_POST['search_keyword']."&a=edit&success=".$result);
		...
		header("Location: ".INDEX_PAGE."events3&a=delete&success=".$result);
		...
		case 'VIEW':
		//header("Location: ".INDEX_PAGE."events");
		header("Location: ".INDEX_PAGE."events3&providername=".$_POST['providername']."&search_keyword=".$_POST['search_keyword']);
		break;
...
$().ready(function() {
	...
	var locsuggest = [
		...
		?>
		{ locid: "<?php echo utf8_encode($locid);?>", location: "<div style=float:left;width:35;margin:right:10px;><?php if ($result['loc_image_path']!= ""){?><img width=30px src=uploads/<?php echo $result['loc_image_path'];?>><?php } ?></div><div style=float:left;> <?php echo utf8_encode($locsuggest);?></div>", locationvalue: "<?php echo trim(utf8_encode($locsuggest));?>" }
		<?php
			}else{
		?>
		{ locid: "<?php echo utf8_encode($locid);?>", location: "<div style=float:left;width:35;margin:right:10px;><?php if ($result['loc_image_path']!= ""){?><img width=30px src=uploads/<?php echo $result['loc_image_path'];?>><?php } ?></div><div style=float:left;> <?php echo utf8_encode($locsuggest);?></div>" , locationvalue: "<?php echo trim(utf8_encode($locsuggest));?>"},
	...
	var leader2suggest = [
		<?php
		$countleader = 0;
		$query = mysql_query("SELECT *,concat(lastname ,', ',firstname, '  ',company) as companyname FROM ".DB_TABLE_PREFIX."leader order by lastname asc");
		$numlocation = mysql_num_rows($query);
			while ($result = mysql_fetch_assoc($query)) {
			$countlocation++;
...
.ac_over {
	...
	<?php
		...
		location.href = '<?php echo INDEX_PAGE."events3&providername=".$_REQUEST['providername']."&search_keyword=".$_REQUEST['search_keyword']?>';
		});
	...
	<tr>
		...
		$("#divTxt1").append("<div id='row1" + id + "' style='margin-bottom:4px;'><div style=float:left;><input style='width:60px;margin-left:9px;' type='text' class='hasDatePicker' title='date' onChange=startdatefunction("+id+") onclick=startdatefunctionclick("+id+") onblur=startdatefunctionblur() name='start_date_" + id + "'  id='start_date_" + id + "'> <input name='end_date_" + id + "' onChange=enddatefunction(" + id + ") onclick=enddatefunctionclick() onblur=enddatefunctionblur() id='end_date_" + id + "' type='text' style='width:60px;margin-left:14px;' value='' /> &nbsp;<a href='#' onClick='removeformField1(\"#row1" + id + "\"); return false;'><img src=images/x.png></a> <a href='#addsizeoption' onClick='addrepitition(" + id + "); return false;' id='addrepitition_" + id + "'>&nbsp;<img src='images/next.jpg' width=16px height=18 border='0' /></a> <a href='#addsizeoption' onClick='deleterepitition(" + id + "); return false;' id='deleterepitition_" + id + "' style=display:none;>&nbsp;<img src='images/icon-delete.png' border='0' /></a></div><table id='repititiondiv_" + id + "' style='display:none;margin:0px;padding:0px;float:left;'><tr><td><select name='dateoption_" + id + "' id='dateoption_" + id + "' style='margin-left:0px;'><?php echo $selectoption;?></select> &nbsp;&nbsp;&nbsp;<select id=fixorduration_" + id + " name=fixorduration_" + id + " onchange=fixorduration(" + id + ")  disabled=disabled><option value=duration><?php echo $durationtimes;?></option><option value=fix><?php echo $planningenddatefield;?></option></select></select>&nbsp;<input name='duration_" + id + "' id='duration_" + id + "' onkeyup='anumber(this)' onblur='anumber(this)' type='text' style='width:30px;text-align:center;' maxlength=3 value='1' /> <input name='planning_end_date_" + id + "'  id='planning_end_date_" + id + "' type='text' style='width:60px;display:none;margin-left:5px;' value='' /></td></tr></table><br style=clear:both;></div>");
