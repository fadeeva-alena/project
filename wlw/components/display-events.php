...
/*echo $sql2 = "select *,e.id as eid from t_event e inner join t_dates d
	...
	if ($_GET['location'] != "locname"){
		$locationx = $_GET['location'];
		 ...
		 $sqlqry1 = "select *,e.id as eid from t_event e inner join t_location l
         on l.id=e.location inner join t_dates x on x.events_id=e.id
		 inner join t_dates d on e.id=d.events_id
		 where (x.events_start_date > '{$datetocheck}') and ((l.loc_name LIKE '%$locationx%' OR l.loc_detail LIKE '%$locationx%' OR l.loc_adress1 LIKE '%$locationx%' OR l.loc_adress2 LIKE '%$locationx%' OR l.loc_zip LIKE '%$locationx%' OR l.loc_loc LIKE '%$locationx%')) $getquality  $getkind $gettype group by e.id limit 20";
		 }else{
...
inner join t_leader le ON e.leader=le.id
			...
			$sqlqry1 = "SELECT e.*,e.id as eid,l.*,le.company,le.firstname,le.lastname FROM t_event e 
INNER JOIN t_dates d ON e.id=d.events_id 
inner JOIN t_location l on l.id=e.location 
inner join t_leader le ON e.leader=le.id
								where (d.events_start_date > '{$datetocheck}')
								AND (e.title LIKE '%$keyword%'
								 OR e.short_desc LIKE '%$keyword%' OR e.long_desc LIKE '%$keyword%' OR e.price LIKE '%$keyword%' OR e.remark_price LIKE '%$keyword%' OR e.remark_prerequisite LIKE '%$keyword%' OR e.date_remark LIKE '%$keyword%' OR e.time_start LIKE '%$keyword%' OR e.time_end LIKE '%$keyword%' OR e.remark_time LIKE '%$keyword%'
								
								OR le.company LIKE '%$keyword%' OR le.firstname LIKE '%$keyword%' OR le.lastname LIKE '%$keyword%')
								and (l.loc_name LIKE '%$locationx%' OR l.loc_detail LIKE '%$locationx%' OR l.loc_adress1 LIKE '%$locationx%' OR l.loc_adress2 LIKE '%$locationx%' OR l.loc_zip LIKE '%$locationx%' OR l.loc_loc LIKE '%$locationx%') $getquality  $getkind $gettype group by e.id limit 20";					
		...
		if ($_GET['keyword'] == ""){
         ...
         on l.id=e.location inner join t_dates x on x.events_id=e.id
		 inner join t_dates d on e.id=d.events_id
		 where (x.events_start_date > '{$datetocheck}') $getquality  $getkind $gettype group by e.id limit 20";
		 }else{
			 $br = strtolower($_SERVER['HTTP_USER_AGENT']); // what browser they use.
			...
			$sqlqry1 = "SELECT *,e.id as eid FROM t_event e 
								INNER JOIN t_dates d
								ON e.id=d.events_id
								INNER JOIN t_leader le
								ON e.leader=le.id
								...
								OR (e.short_desc LIKE '%$keyword%' OR e.long_desc LIKE '%$keyword%' OR e.price LIKE '%$keyword%' OR e.remark_price LIKE '%$keyword%' OR e.remark_prerequisite LIKE '%$keyword%' OR e.date_remark LIKE '%$keyword%' OR e.time_start LIKE '%$keyword%' OR e.time_end LIKE '%$keyword%' OR e.remark_time LIKE '%$keyword%')
								OR (le.company LIKE '%$keyword%' OR le.firstname LIKE '%$keyword%' OR le.lastname LIKE '%$keyword%')) 
								$getquality  $getkind $gettype group by e.id limit 20";					
					
		 }
	}							
	//echo $sqlqry;						
	$sqlevent = mysql_query($sqlqry);
	$sqlevent1 = mysql_query($sqlqry1);
    ...
    <form action="<?php echo INDEX_PAGE . $page_option ?>" method="post" id="frm_<?php echo $page_name?>"  enctype="multipart/form-data">
		...
		if ($_SESSION[WEBSITE_ALIAS]['language'] ==1){
			...
			$commentshover = $rowfield['fieldname_it'];
				...
				?>
				<div style="float:right;">
				<img src="images/comments.png" width="25px" style="cursor:pointer;" class="commenticon" alt="<?php echo $commentshover;?>" title="<?php echo $commentshover;?>"/>
				</div>
				<?php
				}
				?>
...
if ($quality ==1){
	...
	//echo "select * from t_dates where events_id='".$rowevent['eid']."' and events_start_date <='$datestr' and events_end_date>='$datestr'";
...
?>
	...
	<hr>
		<h1 style="font-size:15px;"><?php echo translatefields(830);?></h1>
<?php
//echo $sqlqry1;
$ctr = 0;
while ($rowevent1 = mysql_fetch_array($sqlevent1)){
	$title = $rowevent1['title'];
	$kind = $rowevent1['kind'];
	$type = $rowevent1['type'];
	$short_desc = $rowevent1['short_desc'];
	$long_desc = $rowevent1['long_desc'];
	$location = $rowevent1['location'];
	$price = $rowevent1['price'];
	$currency = $rowevent1['currency'];
	$remark_price = $rowevent1['remark_price'];
	$remark_prerequisite = $rowevent1['remark_prerequisite'];
	$eve_contact_name = $rowevent1['eve_contact_name'];
	$eve_contact_phone = $rowevent1['eve_contact_phone'];
	$eve_contact_email = $rowevent1['eve_contact_email'];
	$eve_contact_url = $rowevent1['eve_contact_url'];
	$eve_loc = $rowevent1['eve_loc'];
	$eve_image_path = $rowevent1['eve_image_path'];
	$provider = $rowevent1['provider'];
	$timestamp = $rowevent1['timestamp'];
	$last_change = $rowevent1['last_change'];
	
	$date_start = $rowevent1['date_start'];
	$date_end = $rowevent1['date_end'];
	
	$date_start = date('d.m.Y',strtotime($rowevent1['date_start']));
	$date_end = date('d.m.Y',strtotime($rowevent1['date_end']));
	
	$date_remark = $rowevent1['date_remark'];
	$time_start = $rowevent1['time_start'];
	$time_end = $rowevent1['time_end'];
	$remark_time = $rowevent1['remark_time'];
	$leader = $rowevent1['leader'];
	$leader2 = $rowevent1['leader2'];
	
	$quality = $rowevent1['quality'];
...
?>
<div class="toggle" style="clear:both;">- <a style="font-weight:bold;font-size;13px;" onclick="javascript:sideClick(<?php echo $counterx;?>);" class="togx" href="javascript:toggleDiv('panel10000<?php echo $ctr;?>',<?php echo $rowevent1['eid'];?>);" ><?php echo fixEncoding($title);?></a></div>

<div style="width:400px;<?php if (mysql_num_rows($sqlevent1) > 1){?>display:none;<?php } ?>margin-top:5px;" id="panel10000<?php echo $ctr;?>" class="panel">
    ...
    <form action="<?php echo INDEX_PAGE . $page_option ?>" method="post" id="frm_<?php echo $page_name?>"  enctype="multipart/form-data">
		...
		<?php $sqlfield = mysql_query("select * from t_field_names where id=681");
				...
				<script type="text/javascript">
				$(document).ready(function() {
					$('#commenticon<?php echo $rowevent1['id'];?>').click(function () {
						$('#commentsform<?php echo $rowevent1['id'];?>').show();
						$('#commenticon<?php echo $rowevent1['id'];?>').hide();
					});
					$('#hideicon<?php echo $rowevent1['id'];?>').click(function () {
						$('#commentsform<?php echo $rowevent1['id'];?>').hide();
						$('#commenticon<?php echo $rowevent1['id'];?>').show();
					});
					$('#submitfeedback<?php echo $rowevent1['id'];?>').click(function () {
						var events_feedback = $('#events_feedback<?php echo $rowevent1['id'];?>').val();
						var leader_feedback = $('#leader_feedback<?php echo $rowevent1['id'];?>').val();
						var location_feedback = $('#location_feedback<?php echo $rowevent1['id'];?>').val();
						var spiritwings_feedback = $('#spiritwings_feedback<?php echo $rowevent1['id'];?>').val();
						
						$('#events_feedback<?php echo $rowevent1['id'];?>').val('');
						$('#leader_feedback<?php echo $rowevent1['id'];?>').val('');
						$('#location_feedback<?php echo $rowevent1['id'];?>').val('');
						$('#spiritwings_feedback<?php echo $rowevent1['id'];?>').val('');
						
						var events_id = $('#events_id<?php echo $rowevent1['id'];?>').val();
						var location_id = $('#location_id<?php echo $rowevent1['id'];?>').val();
						var leader_id = $('#leader_id<?php echo $rowevent1['id'];?>').val();
						var feedback_by = '<?php echo $_SESSION[WEBSITE_ALIAS]['admin_id'];?>';
				...
				<div style="float:left;margin-left:0px;display:none;" id="commentsform<?php echo $rowevent1['id'];?>">
		...
		?></legend>
					<table class="form-table" style="width:350px;">
						<tr>
							<td align="right" colspan="2"><div style="float:right;margin:0px;padding:0px;"><a href="#commenticon<?php echo $rowevent1['id'];?>" id="hideicon<?php echo $rowevent1['id'];?>"><b><?php echo $hideformlabel;?></b></a></div></td>
						</tr>
						<tr>
							<td align="left" colspan="2">
							<?php $sqlfield = mysql_query("select * from t_field_names where id=676");
						...
						<tr>
							...
							?></b></td>
							<td>
								<textarea id="events_feedback<?php echo $rowevent1['id'];?>" style="width:189px;"></textarea>
							</td>
						</tr>
						<tr>
							<td class="key"><b><?php $sqlfield = mysql_query("select * from t_field_names where id=678");
							...
							?></b></td>
							<td>
								<textarea id="location_feedback<?php echo $rowevent1['id'];?>" style="width:189px;"></textarea>
							</td>
						</tr>
						<tr>
							<td class="key"><b><?php $sqlfield = mysql_query("select * from t_field_names where id=679");
							...
							?></b></td>
							<td>
								<textarea id="leader_feedback<?php echo $rowevent1['id'];?>" style="width:189px;"></textarea>
							</td>
						</tr>
						<tr>
							<td class="key"><b><?php $sqlfield = mysql_query("select * from t_field_names where id=680");
							...
							?></b></td>
							<td>
								<textarea id="spiritwings_feedback<?php echo $rowevent1['id'];?>" style="width:189px;"></textarea>
							</td>
						</tr>
						...
						<tr>
							<td></td>
							<td>
							<input type="hidden" id="events_id<?php echo $rowevent1['id'];?>" value="<?php echo $rowevent1['eid'];?>"/>
							<input type="hidden" id="location_id<?php echo $rowevent1['id'];?>" value="<?php echo $location;?>" />
							<input type="hidden" id="leader_id<?php echo $rowevent1['id'];?>" value="<?php echo $leader;?>"/>
							<input type="hidden" id="feedback_by<?php echo $rowevent1['id'];?>" value="<?php echo $_SESSION[WEBSITE_ALIAS]['admin_id'];?>"/>
							<input type="button" class="button" id="submitfeedback<?php echo $rowevent1['id'];?>"
								value="<?php $sqlfield = mysql_query("select * from t_field_names where id=682");
							$rowfield = mysql_fetch_array($sqlfield);
							if ($_SESSION[WEBSITE_ALIAS]['language'] ==1){
								echo $rowfield['fieldname_de'];
		...
		if ($_SESSION[WEBSITE_ALIAS]['language'] ==1){
			...
			$commentshover = $rowfield['fieldname_it'];
					...
					?>
					<div style="float:right;">
				<img src="images/comments.png" width="25px" style="cursor:pointer;" id="commenticon<?php echo $rowevent1['id'];?>" alt="<?php echo $commentshover;?>" title="<?php echo $commentshover;?>"/>
				</div>
				<?php
				}else{
				?>
...
if ($quality ==1){
	...
	$enddatelabel = $rowfield['fieldname_it'];
		...
		$datestr  = date('Y-m-d');			
	$sql = mysql_query("select * from t_dates where events_id='".$rowevent1['eid']."' and events_start_date >='$datestr'");
	//echo "select * from t_dates where events_id='".$rowevent1['eid']."' and events_start_date <='$datestr' and events_end_date>='$datestr'";
	...
	if ($number > 1){
	//echo "<div style='float:left;margin-left:4px;width:65px;border:0px solid red;'><b>".$enddatelabel."</b></div><div style='float:left;width:40px;'><a href='components/download-ical.php?id=".$rowevent1['eid']."'><img src='images/icon-downloads.png' width='16px' height='16px' border='0'/></a> <a onclick=emailevent(1,$rowevent1[eid],'')><img src='images/icon-send-email.png' width='16px' height='16px' border='0'/></a></div><br style=clear:both;>";
	echo "<div style='float:left;margin-left:4px;width:85px;border:0px solid red;'><b>".$enddatelabel."</b></div><br style=clear:both;>";
	}else{
	echo "<div style='float:left;margin-left:4px;width:85px;border:0px solid red;'><b>".$enddatelabel."</b></div><br style=clear:both;>";
	}
	...
	echo "<div style='float:left;margin-left:14px;width:65px;border:0px solid red;'>".date('d.m.Y',strtotime($size_row['events_start_date']))."</div>";
		if ($size_row['events_start_date'] != $size_row['events_end_date']){
			echo "<div style='float:left;margin-left:4px;width:65px;border:0px solid red;'>".date('d.m.Y',strtotime($size_row['events_end_date']))."</div></div><div style='float:left;width:40px;'><a href='components/download-ical.php?id=".$rowevent1['eid']."&date_id=".$size_row['id']."'><img src='images/icon-downloads.png' width='16px' height='16px' border='0'/></a> <a style=cursor:pointer; onclick=emailevent(1,$rowevent1[eid],$size_row[id])><img src='images/icon-send-email.png' width='16px' height='16px' border='0'/></a></div><div id=mailprocessing".$size_row[id]." class=mailprocessing style=float:left;></div><br style=clear:both;>";
		}else{
			echo "<div style='float:left;margin-left:4px;width:65px;border:0px solid red;'>&nbsp;</div></div><div style='float:left;width:40px;'><a style=cursor:pointer; href='components/download-ical.php?id=".$rowevent1['eid']."&date_id=".$size_row['id']."'><img src='images/icon-downloads.png' width='16px' height='16px' border='0'/></a> <a style=cursor:pointer; onclick=emailevent(1,$rowevent1[eid],$size_row[id])><img src='images/icon-send-email.png' width='16px' height='16px' border='0'/></a></div><div id=mailprocessing".$size_row[id]." class=mailprocessing style=float:left;></div><br style=clear:both;>";
		}
	}	
...
if ($_GET['location'] != "locname"){
		$locationx = $_GET['location'];
		 ...
		 inner join t_dates d on e.id=d.events_id
		 where (x.events_start_date <= '{$datetocheck}' and x.events_end_date >= '{$datetocheck}') and ((l.loc_name LIKE '%$locationx%' OR l.loc_detail LIKE '%$locationx%' OR l.loc_adress1 LIKE '%$locationx%' OR l.loc_adress2 LIKE '%$locationx%' OR l.loc_zip LIKE '%$locationx%' OR l.loc_loc LIKE '%$locationx%')) $getquality $getkind $gettype group by e.id";
			$sql1 = "select *,e.id as eid from t_event e inner join t_location l
         on l.id=e.location inner join t_dates x on x.events_id=e.id
		 inner join t_dates d on e.id=d.events_id
		 where (x.events_start_date > '{$datetocheck}') and ((l.loc_name LIKE '%$locationx%' OR l.loc_detail LIKE '%$locationx%' OR l.loc_adress1 LIKE '%$locationx%' OR l.loc_adress2 LIKE '%$locationx%' OR l.loc_zip LIKE '%$locationx%' OR l.loc_loc LIKE '%$locationx%')) $getquality $getkind $gettype group by e.id limit 20";
		 }else{
...
inner join t_leader le ON e.leader=le.id
								...
								OR le.company LIKE '%$keyword%' OR le.firstname LIKE '%$keyword%' OR le.lastname LIKE '%$keyword%')
								and (l.loc_name LIKE '%$locationx%' OR l.loc_detail LIKE '%$locationx%' OR l.loc_adress1 LIKE '%$locationx%' OR l.loc_adress2 LIKE '%$locationx%' OR l.loc_zip LIKE '%$locationx%' OR l.loc_loc LIKE '%$locationx%') $getquality  $getkind $gettype group by e.id";
			$sql1 = "SELECT e.*,e.id as eid,l.*,le.company,le.firstname,le.lastname FROM t_event e 
INNER JOIN t_dates d ON e.id=d.events_id 
inner JOIN t_location l on l.id=e.location 
inner join t_leader le ON e.leader=le.id
								where (d.events_start_date > '{$datetocheck}')
		 ...
		 where (x.events_start_date > '{$datetocheck}') $getquality  $getkind $gettype group by e.id limit 20";
			...
			$sql = "SELECT *,e.id as eid FROM t_event e 
								...
								OR (le.company LIKE '%$keyword%' OR le.firstname LIKE '%$keyword%' OR le.lastname LIKE '%$keyword%')) 
								 ...
								 $sql1 = "SELECT *,e.id as eid FROM t_event e 
								INNER JOIN t_dates d
								ON e.id=d.events_id
								inner JOIN t_leader le
								ON e.leader=le.id
...
while ($row1 = mysql_fetch_array($mysql1)){
 $ctr12++;
 $address2 = $row1['loc_adress1'] . ' ' . $row1['loc_adress2'] . ' ' . $row1['loc_loc'] . ' ' . $row1['loc_zip'] . ' ' . $row1['long'];
 $address3 .= "<b>".$row1['title'] ."</b><br />";
 $address3 .= $row1['loc_adress1'] . ' ' . $row1['loc_adress2'] . '<br />' . $row1['loc_zip'] . ' ' . $row1['loc_loc'] . ' ' . $row1['long'].'<br/>';

$gm->SetAddress(fixEncoding($address2));
$gm->SetInfoWindowText(fixEncoding($address3));
$gm->SetSideClick(fixEncoding($row1[title]));
$address2 = "";
$address3 = "";
}*/
