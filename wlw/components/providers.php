...
session_start();
	...
	<form id="frm_customsearch">
		...
		$rowfield = mysql_fetch_array($sqlfield);
		if ($_SESSION[WEBSITE_ALIAS]['language'] ==1){
			$userlevel = $rowfield['fieldname_de'];
		}elseif ($_SESSION[WEBSITE_ALIAS]['language'] ==2){
			$userlevel =$rowfield['fieldname_eng'];
		}elseif ($_SESSION[WEBSITE_ALIAS]['language'] ==3){
			$userlevel =$rowfield['fieldname_fr'];
		}elseif ($_SESSION[WEBSITE_ALIAS]['language'] ==4){
			$userlevel =$rowfield['fieldname_it'];
		}
...
<link href="plugins/jquery/fancybox/jquery.fancybox-1.3.1.css" rel="stylesheet" type="text/css" /> 
     ...
     {display: '<?php echo $company;?>', name : 'company', width : 100, sortable : true, align: 'left'},
     {display: '<?php echo $firstname;?>', name : 'firstname', width : 80, sortable : true, align: 'left'},
     {display: '<?php echo $lastname;?>', name : 'lastname', width : 80, sortable : true, align: 'left'},
	 {display: '<?php echo $email;?>', name : 'email', width : 130, sortable : true, align: 'left'},
	 {display: '<?php echo str_replace("'","",$rowtotal1);?>', name : 'lastname', width : 60, sortable : false, align: 'center'},
	 {display: '<?php echo str_replace("'","",$rowtotal1);?>', name : 'lastname', width : 60, sortable : false, align: 'center'},
     {display: '<?php echo $photoimg;?>', name : 'image_path', width : <?php echo $grid_max_x+5;?>, sortable : true, align: 'center'},
	{display: '<?php echo $userlevel;?>', name : 'user_level', width : 70, sortable : false, align: 'center'}
     ],
	 buttons : 
     [
		<?php if ($rowsessionrights['provider_w'] == 1){ ?>{name: '<?php echo $add?>', bclass: 'add', onpress : buttonAction}<?php } ?>
     ],
     sortname: "<?php echo $fg_co_workers['sortname']?>",
     sortorder: "<?php echo $fg_co_workers['sortorder']?>",
     usepager: true,
     title: '<?php echo str_replace("'","",$page_heading);?>',
     useRp: true,
     rp: <?php echo $fg_co_workers['rp']?>,
     rpOptions: <?php echo $fg_co_workers['rpOptions']?>,
     showTableToggleBtn: <?php echo $fg_co_workers['showTableToggleBtn']?>,
     width: '<?php echo $fg_co_workers['width']?>',
     height: 'auto',
     resizable: <?php echo $fg_co_workers['resizable']?>,
