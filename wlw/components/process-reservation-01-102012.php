...
$sqlfield = mysql_query("select * from t_field_names where id=410");
	...
	echo "<a style=cursor:pointer; onclick='datereserveagain(1,$size_row[did],$provider_id)'><img src='images/reservation.png' width='16px' height='16px' border='0'/></a> ";					
...
$getnum = mysql_query("select c.currency as ccurrency,e.*,d.id as did from t_dates d inner join t_event e on e.id=d.events_id
	...
	$count = 0;
	$up = 0;
	$checkupordown = mysql_query("select * from t_reservations where date_id='".$dateid."' and provider_id='$provider_id'");
	
	//echo "select * from t_reservations where date_id='".$dateid."' and provider_id='$provider_id'";
	while ($result= mysql_fetch_array($checkupordown)){
		$count++;
		if (($count <= $size_row['max_number']) and ($result['date_id'] == $dateid)){
			$up++;
		}
	}
	
	$checkupordown = mysql_query("select * from t_reservations where date_id='".$dateid."' and provider_id='$provider_id' and (reservation_status=1 or reservation_status=1) order by reservation_id desc");
	...
	if ($up > 0){	
		...
		$getifyes = mysql_query("select * from t_reservations where date_id='$dateid' and (reservation_status=0 or reservation_status=1)");	
		if (mysql_num_rows($getifyes) > $size_row['max_number']){
