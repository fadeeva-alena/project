...
$tablename = DB_TABLE_PREFIX.'event_kind';
...
switch ($form_action)
{
	case 'ADD':	
		...
		header("Location: ".INDEX_PAGE."events-kind&a=add&mode=view&success=".$result."&id=".$id);
		break;
	...
	case 'EDIT':		
		...
		header("Location: ".INDEX_PAGE."events-kind&a=edit&mode=edit&success=".$result);
		break;
	...
	case 'DELETE':
		if ( (strtoupper($_POST["Delete"]) == 'YES') && ($id != 1) ) {
			...
			header("Location: ".INDEX_PAGE."events-kind&a=delete&success=".$result);
	...
	case 'VIEW':
		header("Location: ".INDEX_PAGE."events-kind");
		break;
...
$record = $sql_helper->cget_row(DB_TABLE_PREFIX."event_kind", "id = '$id'") ;


$kind_de = $record->kind_de;
$kind_eng = $record->kind_eng;
$kind_fr = $record->kind_fr;
$kind_it = $record->kind_it;
...
$(document).ready(function() {
	...
	$('#btnCancel').click(function () {
		location.href = '<?php echo INDEX_PAGE."events-kind"?>';
	});
...
<div class="content-main wide70">
    ...
    <form action="<?php echo INDEX_PAGE . $page_option; ?>" method="post" id="frm_<?php echo $page_name; ?>" enctype="multipart/form-data">
		...
		$sqlfield = mysql_query("select * from t_field_names where id=340");
		$rowfield = mysql_fetch_array($sqlfield);
		if ($_SESSION[WEBSITE_ALIAS]['language'] ==1){
			echo $rowfield['fieldname_de'];
		}elseif ($_SESSION[WEBSITE_ALIAS]['language'] ==2){
		...
		<?php echo $req_fld?></label></td>
                    <?php if ( $is_editable_field ) { ?>
                    <td>
                    	<input type="text" name="kind_de" id="kind_de" style="width:190px;" maxlength="50" value="<?php echo fixEncoding($kind_de)?>" />
					</td>
                    <?php } else { ?>
                    <td><?php echo $kind_de?></td>
                    <?php } ?>                                                                                                    
                </tr>
                
                <tr>
                    <td class="key"><label for="nick"><?php
		$sqlfield = mysql_query("select * from t_field_names where id=341");
		$rowfield = mysql_fetch_array($sqlfield);
		if ($_SESSION[WEBSITE_ALIAS]['language'] ==1){
			echo $rowfield['fieldname_de'];
		}elseif ($_SESSION[WEBSITE_ALIAS]['language'] ==2){
		...
		<?php echo $req_fld?></label></td>
                    <?php if ( $is_editable_field ) { ?>
                    <td>
                    	<input type="text" name="kind_eng" id="kind_eng" style="width:190px;" maxlength="50" value="<?php echo fixEncoding($kind_eng)?>" />
					</td>
                    <?php } else { ?>
                    <td><?php echo $kind_eng?></td>
                    <?php } ?>                                                                                                    
                </tr>
                
                <tr>
                    <td class="key"><label for="nick"><?php
		$sqlfield = mysql_query("select * from t_field_names where id=342");
		$rowfield = mysql_fetch_array($sqlfield);
		if ($_SESSION[WEBSITE_ALIAS]['language'] ==1){
			echo $rowfield['fieldname_de'];
		}elseif ($_SESSION[WEBSITE_ALIAS]['language'] ==2){
		...
		<?php echo $req_fld?></label></td>
                    <?php if ( $is_editable_field ) { ?>
                    <td>
                    	<input type="text" name="kind_fr" id="kind_fr" style="width:190px;" maxlength="50" value="<?php echo fixEncoding($kind_fr)?>" />
					</td>
                    <?php } else { ?>
                    <td><?php echo $kind_fr?></td>
                    <?php } ?>                                                                                                    
                </tr>
                
                <tr>
                    <td class="key"><label for="nick"><?php
		$sqlfield = mysql_query("select * from t_field_names where id=343");
		$rowfield = mysql_fetch_array($sqlfield);
		if ($_SESSION[WEBSITE_ALIAS]['language'] ==1){
			echo $rowfield['fieldname_de'];
		}elseif ($_SESSION[WEBSITE_ALIAS]['language'] ==2){
		...
		<?php echo $req_fld?></label></td>
                    <?php if ( $is_editable_field ) { ?>
                    <td>
                    	<input type="text" name="kind_it" id="kind_it" style="width:190px;" maxlength="50" value="<?php echo fixEncoding($kind_it)?>" />
					</td>
                    <?php } else { ?>
                    <td><?php echo $kind_it?></td>
                    <?php } ?>                                                                                                    
                </tr>
