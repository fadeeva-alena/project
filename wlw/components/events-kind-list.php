...
$rowsessionrights = mysql_fetch_array($sqlsessionrights);
	...
	$detail_max_y = $row_sys['pics_in_detail_max_y'];
     ...
     $table_name = DB_TABLE_PREFIX."event_kind";
     $page = $_POST['page'];
     $rp = $_POST['rp'];
     $sortname = $_POST['sortname'];
     $sortorder = $_POST['sortorder'];
     
     if (!$sortname) $sortname = $fg_event_kind['sortname'];
     if (!$sortorder) $sortorder = $fg_event_kind['sortorder'];
   ...
   $column = $_POST['column'];
	...
	if ( ($search_keyword != '') || ($column != '') ) {
		if ( $column != '' ) {
			...
			$field_names = array('kind_de','kind_eng','kind_fr','kind_it');
		}					
    ...
    $operation_url = INDEX_PAGE.'events-kind-m&mode=';
    $rc = false;	
    while ($row = mysql_fetch_array($result))
    {
        ...
        $json .= "id:'".$record_id."',";
        $json .= "cell:['".$action."'";
        $json .= ",'".fixEncodingx($row['kind_de'])."'";
        $json .= ",'".fixEncodingx($row['kind_eng'])."'";
        $json .= ",'".fixEncodingx($row['kind_fr'])."'";
        $json .= ",'".fixEncodingx($row['kind_it'])."']";
        $json .= "}";
        $rc = true;
    }
    $json .= "]\n";
