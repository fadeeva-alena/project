...
error_reporting(0);
session_start();
$sqlfield = mysql_query("select * from t_field_names where id=636");
...
?>
	...
	?>
		...
		*/
		...
		if ($_GET['search_keyword'] != ""){
			...
			$search_keyword = $_SESSION['events_searchkeyword2'];
		}
     ...
     {display: '<?php echo $name;?>', name : '', width : 180, sortable : true, align: 'left'},
    ...
    // {display: '<?php echo $address2;?>', name : 'loc_adress2', width : 93, sortable : true, align: 'center'},
	 {display: '<?php echo $contact;?>', name : '', width : 180, sortable : true, align: 'left'},
			...
			if (numSelected > 0){
				...
				$.ajax 
					({
						type: "POST",
						dataType: "json",
						url: "<?php echo PATH_COMPONENTS?>process-handover-locations.php?locationsid="+ allVals,
