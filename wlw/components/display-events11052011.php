...
if ($quality != ""){
	...
	$getquality = " and quality in ($val)";
}
else{
	$getquality = " and quality in (1111111111111111111)";
}
// Retrieve record
//if(!empty($id) || $id != '') :
...
<table style="margin-bottom:10px;margin-top:-10px;background-color:white;">
	<?php
	if ($_SESSION[WEBSITE_ALIAS]['language'] ==1){
...
<div class="toggle" style="clear:both;">- <a style="font-weight:bold;font-size;13px;" class="togx" onclick="javascript:sideClick(<?php echo $counterx;?>);" href="javascript:toggleDiv('panel<?php echo $ctr;?>');" ><?php echo fixEncoding($title);?></a></div>
...
if ($quality ==1){
			...
			<?php
				...
				<tr>
					...
					?> Not in the list? Click <a href="index2.php?option=locations2-m&mode=add" class="modalpopup1">
					<a href="javascript:void(0);" onclick="PopupCenter('index2.php?option=leaders2-m&mode=view&view=view&id=<?php echo $leader;?>', 'myPop1',1000,500);" class="modalpopup2">
					here</a>.
					<span class="validation-status"></span> </td>
					<?php } else { ?>
					<td><?php 
	...
	echo "<div style='float:left;margin-left:4px;width:85px;border:0px solid red;'>".date('d.m.Y',strtotime($size_row['events_end_date']))."</div><br style=clear:both;>";
	}	
	?>					
					</td>
			...
			<?php
				...
				?>Not in the list? Click <a href="index2.php?option=leaders2-m&mode=add" class="modalpopup2">here</a>.
          		<span class="validation-status"></span> </td>
        		<?php } else { ?>
        		<td><?php 
                             if ($leader != ""){
...
inner join t_dates d on e.id=d.events_id
  where $where (d.events_start_date <= '{$datetocheck}' and d.events_end_date >= '{$datetocheck}') $getquality GROUP BY e.id");
 $sql2 = "select *,e.id as eid from t_event e inner join t_dates d
								on e.id=d.events_id where $where (events_start_date <= '{$datetocheck}' and events_end_date >= '{$datetocheck}') $getquality GROUP BY e.id $order";
 $address = "";
 while ($row = mysql_fetch_array($sql)){
 $ctr12++;
 ...
 $address .= $row['loc_adress1'] . ' ' . $row['loc_adress2'] . '<br />' . $row['loc_zip'] . ' ' . $row['loc_loc'] . ' ' . $row['long'];
...
?>
<div style="float:right;border:1px solid #eeeeee;width:300px;margin-left:10px;height:auto;margin-top:5px;">
