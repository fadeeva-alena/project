...
$_SESSION['events_providername2']= $providername;
    ...
    $sql = "SELECT l.*,c.*,
		...
		hlocations.id as main_record_id FROM $table_name l inner join t_country c
		...
		$where $userlevelgain and request_status='for approval' and hlocations.provider_id!='".$_SESSION[WEBSITE_ALIAS]['admin_id']."' $sort $limit";
    ...
    $total = $sql_helper->sql_count("SELECT count(*) FROM $table_name l inner join t_country c
		...
		inner join t_handover_locations hlocations on hlocations.location_id=l.id
		inner join t_provider provider on provider.id=hlocations.provider_id
		$where $userlevelgain and hlocations.provider_id!='".$_SESSION[WEBSITE_ALIAS]['admin_id']."'");
