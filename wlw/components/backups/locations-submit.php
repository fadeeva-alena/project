...
$post_data['created'] = date('Y-m-d');
	...
	$sql = mysql_query("select * from t_location order by loc_name asc");	
	echo '<option value="">- Select Location -</option>'; 
	while ($row = mysql_fetch_array($sql)){
		if ($id == $row['id']){
			echo '<option value="'.$row['id'].'" selected=selected>'.$row['loc_name'] . ', '. $row['loc_detail'].'</option>';
		}else{
			echo '<option value="'.$row['id'].'">'.$row['loc_name'] . ', '. $row['loc_detail'].'</option>';
		}
	}
