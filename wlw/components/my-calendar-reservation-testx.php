...
//sending an email here
			...
			$bodyemail = str_replace("<firstname> <lastname>",genderfields($gender) . " " . $row->firstname ." " . $row->lastname.",",$bodyemail);
...
if (isset($_REQUEST['mode'])){
	...
	if (isset($_POST['mode']) and $_POST['mode'] == "add"){
		...
		header("Location: ".INDEX_PAGE."my-calendar-reservation-test&a=add&success=true");
		exit();
	}
	if (isset($_POST['mode']) and $_POST['mode'] == "edit"){
		if (isset($_POST['provider_event_title'])){
			...
			header("Location: ".INDEX_PAGE."my-calendar-reservation-test&a=edit&success=true");
			exit();
		}
	}
	if (isset($_REQUEST['mode']) and $_REQUEST['mode'] == "delete"){
		...
		header("Location: ".INDEX_PAGE."my-calendar-reservation-test&a=delete&success=true");
		exit();
	}
							...
							$.ajax({
			  				...
			  				success: function(html){
								...
								if (html == "overlap"){
									...
									if (!confirm("<?php echo translatefields(728);?>")) {
										var block_type = "0";
									}else{
										var block_type = "1";
									}
										$.ajax({
											url: "components/provider-reservation.php?provider_id=<?php echo $_SESSION[WEBSITE_ALIAS]['admin_id'];?>&provider_event_title="+copiedEventObject.title+"&start_date="+resdate+"&block_type="+block_type,
											cache: false,
											success: function(html){
												//alert(resdate);
												document.location.href='index.php?option=my-calendar-reservation-test';
											}
										})
					}
					}
			...
			?>
				{
					...
					alldescriptions: '<b><?php echo $rowreservations["provider_event_title"];?></b><br/><?php echo fixEncoding($rowreservations["reservation_start_date"]);?> - <?php echo fixEncoding($rowreservations["reservation_end_date"]);?><br/><?php echo fixEncoding($rowreservations["provider_event_description"]);?><br/><b><?php echo translatefields(8);?></b>: <?php echo $rowreservations['provider_event_price'] . " " . $rowreservations['currency'];?><br /><br /><form method=post action=index.php?option=my-calendar-reservation-test&reservation_id=<?php echo $rowreservations["reservation_id"];?>><table><tr><td colspan=2><hr style="border:1px solid dashed;"></td></tr><tr><td><b><?php echo translatefields(629);?>:</b></td><td><textarea name="reason" id="reason" style=width:300px></textarea></td></tr><tr><td colspan=2 align=right><input type=submit name=submit2 value="<?php echo translatefields(410);?>"></td></tr></table></form>',
					id:'<?php echo fixEncoding($rowreservations["reservation_id"]);?>',
					allDay: false,
					color: '<?php if ($rowreservations['block_type']==''){ echo '#'. fixEncoding($rowreservations["provider_event_color"]);} else { echo '#eeeeee';}?>',
					<?php if ($rowreservations['block_type']=='1'){ ?>
					textColor:'#eeeeee',
					<?php } ?>
					editable: true,
...
?>
	<div class='external-event' price="<?php echo translatefields(8);?></b>: <?php echo $rowproviderevents['provider_event_price'] . " " . $rowproviderevents['currency'];?>" title="<?php echo $rowproviderevents['provider_event_description'];?><br/><b><?php echo translatefields(8);?></b>: <?php echo $rowproviderevents['provider_event_price'] . " " . $rowproviderevents['currency'];?>" <?php if ($rowproviderevents['provider_event_color'] != ""){echo 'style="'.$textcolor.'background-color:#'.$rowproviderevents['provider_event_color'].';"';}?>><a href="index.php?option=my-calendar-reservation-test&mode=edit&provider_event_id=<?php echo $rowproviderevents['provider_event_id'];?>#eventform"><image src="images/icon-edit.png" border="0" /></a> <a onclick="if(!confirm('<?php $str = translatefields(273);echo str_replace("%date_from% - %date_to%","",$str);?>')){return false;}" href="index.php?option=my-calendar-reservation-test&mode=delete&provider_event_id=<?php echo $rowproviderevents['provider_event_id'];?>)"><image src="images/icon-delete.png" border="0" /></a> <?php echo $rowproviderevents['provider_event_title'];?></div>
<?php
	 if ($_REQUEST['provider_event_id'] != "" and $_REQUEST['mode'] != ""){
		...
		<form method="post" action="index.php?option=my-calendar-reservation-test" id="frmeventmgmt">
...
?>
<a  href="index.php?option=my-calendar-reservation-test&mode=add#eventform"><image src="images/add.png" border="0"/> <?php echo translatefields(105);?></a>
<script type="text/javascript">
