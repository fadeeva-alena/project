...
$_SESSION['events_providername1']= $providername;
   $column = $_POST['column'];
	...
	$sql = "SELECT l.*,
		...
		$where $userlevelgain and request_status='for approval' and hleaders.provider_id!='".$_SESSION[WEBSITE_ALIAS]['admin_id']."'  $sort $limit";
    ...
    $totalnum = mysql_query("SELECT * from $table_name l
		inner join t_handover_leaders hleaders on hleaders.leader_id=l.id
		inner join t_provider provider on provider.id=hleaders.provider_id
		$where $userlevelgain and request_status='for approval' and hleaders.provider_id!='".$_SESSION[WEBSITE_ALIAS]['admin_id']."'");
