	...
	switch ($form_action)
	{
		...
		if ($date_start != ""){
			...
			if ($_POST['dateoption_' . $count] == "daily"){
				...
				$timeStamp += 24 * 60 * 60 * ($_POST['duration_' . $count]-1);
				...
				$day_name =  date('l', strtotime($date_start));
				for ($dx =1;$dx <$daysinweeks;$dx++){
					$timeStamp = "";
					$timeStamp = strtotime($date_start);
					$timeStamp += 24 * 60 * 60 * $dx;
					$day_name_compare = date("l",$timeStamp);
					if ($day_name == $day_name_compare){
				...
				$daysinmonths = ($_POST['duration_'. $count]-1) * 31;
				$datestartmonth = date('Y-m-d',strtotime($date_start));
				mysql_query("insert into t_dates values ('0','$id','$datestartmonth','$datestartmonth')");
				$day_name =  date('d', strtotime($date_start));
				for ($dx =1;$dx <=$daysinmonths;$dx++){
					$timeStamp = "";
					$timeStamp = strtotime($date_start);
					$timeStamp += 24 * 60 * 60 * $dx;
					$day_name_compare = date("d",$timeStamp);
					if ($day_name == $day_name_compare){
						$startenddatemonth = date('Y-m-d',$timeStamp);
						//if ($date_start != ""){
							mysql_query("insert into t_dates values ('0','$id','$startenddatemonth','$startenddatemonth')");
						//}
					}
		...
		header("Location: ".INDEX_PAGE."events2&a=add&success=".$result);
		break;
		...
		//header("Location: ".INDEX_PAGE."events2&a=edit&success=".$result);
		
		header("Location: ".INDEX_PAGE."events2&providername=".$_POST['providername']."&search_keyword=".$_POST['search_keyword']."&a=edit&success=".$result);
		...
		header("Location: ".INDEX_PAGE."events2&a=delete&success=".$result);
		...
		case 'VIEW':
		//header("Location: ".INDEX_PAGE."events");
		header("Location: ".INDEX_PAGE."events2&providername=".$_POST['providername']."&search_keyword=".$_POST['search_keyword']);
		break;
...
.ac_over {
	...
	<?php
		...
		location.href = '<?php echo INDEX_PAGE."events2&providername=".$_REQUEST['providername']."&search_keyword=".$_REQUEST['search_keyword']?>';
		});
	...
	<tr>
		...
		echo "<div style='float:left;margin-left:5px;width:75px;border:0px solid red;'><b>".$optiondates."</b> ".$req_fld."</div><div style='float:left;margin-left:5px;width:80px;border:0px solid red;'><b>".$duration."</b> ".$req_fld."</div><br style=clear:both;>";
		...
		<?php
		echo "&nbsp;&nbsp;&nbsp;<input name='duration_1' id='duration_1' onkeyup='anumber(this)' onblur='anumber(this)' type='text' style='width:60px;text-align:center;' maxlength=3 value='1'  /> <a href='#addsizeoption' onClick='addformField2(); return false;' id='addsizeoption'>&nbsp;<img src='images/add.png' border='0' /></a>";
		echo "</div>";
		...
		$("#divTxt1").append("<div id='row1" + id + "' style='display:none;margin-bottom:4px;'><input style='width:60px;margin-left:12px;' type='text' class='hasDatePicker' title='date' onChange=startdatefunction("+id+") onclick=startdatefunctionclick("+id+") onblur=startdatefunctionblur() name='start_date_" + id + "'  id='start_date_" + id + "'> <select name='dateoption_" + id + "' id='dateoption_" + id + "' style='margin-left:15px;'><?php echo $selectoption;?></select> &nbsp;&nbsp;&nbsp;<input name='duration_" + id + "' id='duration_" + id + "' type='text' style='width:60px;text-align:center;' maxlength=3 value=1 onkeyup='anumber(this)' onblur='anumber(this)' />&nbsp;&nbsp;<a href='#' onClick='removeformField1(\"#row1" + id + "\"); return false;'><img src=images/x.png></a></div>");
