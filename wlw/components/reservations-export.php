			...
			$testtoexport .='<tbody>';
				...
				$testtoexport .='<tr>
					...
					if ($rowfields['reservation_status'] == 0){
						...
						/*$cancellation_date = $rowfields['cancellation_date'];
						$date2 = date("m/d/Y", strtotime($size_row['events_start_date']));
						$date1 = date("m/d/Y", strtotime($rowfields['cancellation_date']));
						$results = dateDiff("/", $date2, $date1);
						if (($results <= $size_row['cancellation_day1'])){
							$feeamount = $size_row['cancellation_fee1'];
						}else{
							$feeamount = $size_row['cancellation_fee2'];
						}
						
						excl: i booked an event on 8th (testes.. 3,5 as cancelldays), xls-download shows me 15eur for cancellation. should be 0. (>5 days away, email is stating: no fee).
						same for 7.feb (5days away): email states: 15eur. xls states: none.
						should be 15.
						same for 6.feb: (4 days away): email states: 15: fine. xls states: 0. should be 15.
						same for 5.feb: (3 days away). email states: 15: fine. xls states: none should be 15.
						same for 4.feb (2 days away): email states 15 eur. should be 20.
						xls states: none. should be 20.
						same for 3.feb: (1 day away) email states: 15 eur. should be 20. xls states: none. should be 20.
						same for 2.2.2012: (today): email states: 15 eur. shuld be 20. xls states: 0. shlud be 20.
						...
						*/
							...
							$today = $rowfields['cancellation_date'];
							$results = 0;
					...
					if ($rowfields['rnote'] == "" or $rowfields['rnote'] =="null" or $rowfields['rnote'] == null){
						$rnote = "";
					}else{
						$rnote = $rowfields['rnote'];
					}
					
					$testtoexport .='<td align=center style="border:1px solid #eeeeee;">'.$rnote.'</td>';
