...
$post_data['provider'] = $_SESSION[WEBSITE_ALIAS]['admin_id'];
	...
	$sql = mysql_query("select * from t_leader  order by lastname asc");	
	echo '<option value="">- Select Leader -</option>'; 
	while ($row = mysql_fetch_array($sql)){
		$companyname = $row['company'] . " " . $row['firstname'] . " " . $row['lastname'];
		if ($id == $row['id']){
			echo '<option value="'.$row['id'].'" selected=selected>'.$companyname.'</option>';
		}else{
			echo '<option value="'.$row['id'].'">'.$companyname.'</option>';
		}
	}
