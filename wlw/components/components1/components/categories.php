...
<div id="system-message">
    <div class="info">
        <div class="message"><?php echo $helper->operation_msg($_GET['a'], $_GET['success'], "Category")?></div>
    </div>
</div>
<?php } ?>
...
<div id="custom-search-form" class="align-right">
	<form id="frm_customsearch">
        ...
        <p>
		<?php
		$options_col = array('Category');
		$values_col = array('category_name');
...
<script type="text/javascript">
	...
	({
		...
		colModel : 
			...
			{display: 'Category Name', name : 'category_name', width : 500, sortable : true, align: 'left'},
			{display: 'Active', name : 'is_active', width : 100, sortable : true, align: 'center'}
			],
		buttons : 
			[
		...
		sortname: "<?php echo $fg_categories['sortname']?>",
		sortorder: "<?php echo $fg_categories['sortorder']?>",
		usepager: true,
		title: '<?php echo $page_heading?>',
		useRp: true,
		rp: <?php echo $fg_categories['rp']?>,
		rpOptions: <?php echo $fg_categories['rpOptions']?>,
		showTableToggleBtn: <?php echo $fg_categories['showTableToggleBtn']?>,
		width: '<?php echo $fg_categories['width']?>',
		height: <?php echo $fg_categories['height']?>,
		resizable: <?php echo $fg_categories['resizable']?>,
			...
			if($('.trSelected',grid).length>0)
			{
				if(confirm('Delete ' + $('.trSelected',grid).length + ' record(s)?'))
				{
					...
					({
						type: "POST",
						dataType: "json",
						url: "<?php echo PATH_COMPONENTS?>delete.php?tn=<?php echo urlencode($crypt->encrypt('categories'))?>&fn=<?php echo urlencode($crypt->encrypt('category_id'))?>",
						data: "items="+itemlist,
