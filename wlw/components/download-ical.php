 ...
 */
 ...
 if ($row1['loc_name'] != ""){
	$description .= fixEncoding123($row1['loc_name'] . " " .$row1['loc_detail']) . "<br>";
 }
 if ($row1['loc_adress1'] != ""){
	$description .= fixEncoding123($row1['loc_adress1']) . "<br>";
 }
 if ($row1['loc_adress2'] != ""){
	$description .= fixEncoding123($row1['loc_adress2']) . "<br>";
 }
 if ($row1['loc_zip'] != ""){
	$description .= fixEncoding123($row1['loc_zip'] . " " .$row1['loc_loc']);
 }
 
 //$description .= fixEncoding123($row1['loc_name'] . " " .$row1['loc_detail'] ."<br>" .$row1['loc_adress1'] . '<br>' . $row1['loc_adress2'] ."<br>". $row1['loc_zip'] . " " .$row1['loc_loc']);
 }else{
 $description .= "";
 }
 $description .='</td>';
...
$description .='
 ...
 if ($eve_contact_name != "" or $eve_contact_phone != "" or $eve_contact_email != "" or $eve_contact_url != ""){
 $description .='<tr>
 ...
 $description .=':</b> </label></td>
 <td>'.fixEncoding123($eve_contact_name);
 }
 if ($eve_contact_phone != ""){
 $description .='<br> '.fixEncoding123($eve_contact_phone);
 }
 if ($eve_contact_email != ""){
 $description .='<br> '.fixEncoding123($eve_contact_email);
 }
 if ($eve_contact_url != ""){	
 $description .='<br> '.fixEncoding123($eve_contact_url);
 }
 $description .="</td></tr>";
 if ($eve_image_path != ""){
 $description .='<tr>
...
if ($_GET['date_id'] != ""){
	...
	//$icaldetails .="DTEND;TZID=CET:".date('Ymd',strtotime($rowquery['events_end_date']))."T".date('His',strtotime($rowquery['time_end']))."Z\n";
	$icaldetails .="DTEND;TZID=CET:"  . date('Ymd',strtotime($rowquery['events_end_date']))."T".date('His',strtotime($rowquery['time_end']))."\n";
	}else{
	//$icaldetails .="DTEND;TZID=CET:".date('Ymd',strtotime($rowquery['events_end_date']))."T180000Z\n";
		if ($rowquery['time_start'] != "00:00:00"){
			$icaldetails .="DTEND;TZID=CET:"  . date('Ymd',strtotime($rowquery['events_end_date']))."T".date('His',strtotime($rowquery['time_start']))."\n";
		}else{
			$icaldetails .="DTEND;TZID=CET:"  . date('Ymd',strtotime($rowquery['events_end_date']))."\n";
		}
	}
	
	
	if ($rowquery['time_start'] != "00:00:00"){
	//$icaldetails .="DTSTART;TZID=CET:".date('Ymd',strtotime($rowquery['events_start_date']))."T".date('His',strtotime($rowquery['time_start']))."Z\n";
	
	$icaldetails .="DTSTART;TZID=CET:" .  date('Ymd',strtotime($rowquery['events_start_date']))."T".date('His',strtotime($rowquery['time_start']))."\n";
	
	}else{
		if ($rowquery['time_end'] != "00:00:00"){
			$icaldetails .="DTSTART;TZID=CET:" .  date('Ymd',strtotime($rowquery['events_start_date']))."T".date('His',strtotime($rowquery['time_end']))."\n";
		}else{
			$icaldetails .="DTSTART;TZID=CET:" .  date('Ymd',strtotime($rowquery['events_start_date']))."T000000\n";
		}
	}
	$icaldetails .="DTSTAMP;TZID=CET:".date('Ymd',strtotime($rowquery['etimestamp']))."T".date('His',strtotime($rowquery['etimestamp']))."\n";
	$icaldetails .="LAST-MODIFIED;TZID=CET:".date('Ymd',strtotime($rowquery['elast_change']))."T".date('His',strtotime($rowquery['elast_change']))."\n";
	...
	$icaldetails .='ORGANIZER;CN="Spiritwings Team":mailto:kontakt@spiritwings.ch\n';
	...
	while ($alldatesrow = mysql_fetch_array($alldatessql)){
		...
		$icaldetails .="DTEND:".date('Ymd',strtotime($alldatesrow['events_end_date']))."\n";
		}
		if ($rowquery['time_start'] != "00:00:00"){
...
//Create Mime Boundry
	...
	$headers = "From: Spiritwings Team <kontakt@spiritwings.ch>\n";
	$headers .= "Reply-To: Spiritwings Team <kontakt@spiritwings.ch>\n";
