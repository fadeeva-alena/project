	...
	echo "<div style='float:left;margin-left:14px;width:65px;border:0px solid red;'>".date('d.m.Y',strtotime($size_row['events_start_date']))."</div>";
	  if ($_SESSION[WEBSITE_ALIAS]['admin_id'] ==""){
		...
		if ($size_row['events_start_date'] != $size_row['events_end_date']){
			...
			$checkreservation = mysql_query("select * from t_reservations where date_id='".$size_row['id']."' and provider_id='".$_SESSION[WEBSITE_ALIAS]['admin_id']."'");
			if (mysql_num_rows($checkreservation) > 0){
    			echo "<img src='images/reservation.png' width='16px' height='16px' border='0'/> 
				<img src='images/cancel.png' width='16px' height='16px' border='0'/>";
			}else{
				echo "<img src='images/reservation.png' width='16px' height='16px' border='0'/> ";
			}
			echo "</div><div id=mailprocessing".$size_row[id]." class=mailprocessing style=float:left;></div><br style=clear:both;>";
		}else{
			echo "<div style='float:left;margin-left:4px;width:65px;border:0px solid red;'>&nbsp;</div></div><div style='float:left;width:40px;'><a style=cursor:pointer; href='components/download-ical.php?id=".$rowevent['eid']."&date_id=".$size_row['id']."'><img src='images/icon-downloads.png' width='16px' height='16px' border='0'/></a> <a style=cursor:pointer; onclick=emailevent(1,$rowevent[eid],$size_row[id])><img src='images/icon-send-email.png' width='16px' height='16px' border='0'/></a>";$checkreservation = mysql_query("select * from t_reservations where date_id='".$size_row['id']."' and provider_id='".$_SESSION[WEBSITE_ALIAS]['admin_id']."' and (status=1 or status=0)");
			if (mysql_num_rows($checkreservation) > 0){
				...
				while ($result= mysql_fetch_array($checkupordown)){
					$count++;
					if (($count <= $size_row['max_number']) and ($result['date_id'] == $size_row['id']) and ($result['provider_id'] == $provider_id)){
						$up++;
					}
				}
