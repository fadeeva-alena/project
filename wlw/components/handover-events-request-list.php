...
$sql = "SELECT *,c.currency as currencyal,loc.loc_name as locationval,e.id as eid,
		...
		$where $userlevelgain $userlevelgainfilter and request_status='for approval' and hevents.provider_id!='".$_SESSION[WEBSITE_ALIAS]['admin_id']."' group by e.id $sort $limit";
...
$result = mysql_query($sql);
 ...
 $sql2 = "SELECT *,c.currency as currencyal,loc.loc_name as locationval,e.id as eid FROM $table_name 
		...
		inner join t_handover_events hevents on hevents.events_id=e.id
		inner join t_provider provider on provider.id=hevents.provider_id
		$where $userlevelgain $userlevelgainfilter and request_status='for approval' and hevents.provider_id!='".$_SESSION[WEBSITE_ALIAS]['admin_id']."' group by e.id";
...
while ($row = mysql_fetch_array($result))
{
	...
	$requestorname = "";
	if ($row['pcompany'] != ""){
		$requestorname .= fixEncodingx($row['pcompany']) . "<br />";
	}
	if ($row['pfirstname'] != ""){
	...
	$json .= "id:'".$record_id."',";
	$json .= "cell:['".$action."'";
	$json .= ",'"."<div style=margin:0px;padding:0px;>".$requestorname."</div>'";
	...
	$datestart = "";
	$dateend = "";
	$sqldate = mysql_query("select * from t_dates where events_id='".$row['eid']."'");
