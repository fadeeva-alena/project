...
$lastDayOfTheMonth   = date('Y-m-d',mktime(0,0,0,$currentMonthNumeric + 1,0,$currentFullYear));
   ...
   echo $lastDayOfTheMonth .PHP_EOL;*/
	...
	for ($i = 1;$i<=$duration;$i++){
		...
		header("Location: ".INDEX_PAGE."events3&providername=".$_POST['providername']."&search_keyword=".$_POST['search_keyword']."&a=add&success=".$result);
		break;
		...
		header("Location: ".INDEX_PAGE."events3&providername=".$_POST['providername']."&search_keyword=".$_POST['search_keyword']."&a=delete&success=".$result);
...
$().ready(function() {
	...
	var locsuggest = [
		...
		?>
		{ locid: "<?php echo utf8_encode($locid);?>", location: "<div style=float:left;width:35;margin:right:10px;><?php if ($result['loc_image_path']!= ""){?><img width=30px src=uploads/<?php echo $result['loc_image_path'];?>><?php } ?></div><div style=float:left;> <?php echo utf8_encode($locsuggest);?></div>", locationvalue: "<?php echo trim(utf8_decode($locsuggest));?>" }
		<?php
			}else{
		?>
		{ locid: "<?php echo utf8_encode($locid);?>", location: "<div style=float:left;width:35;margin:right:10px;><?php if ($result['loc_image_path']!= ""){?><img width=30px src=uploads/<?php echo $result['loc_image_path'];?>><?php } ?></div><div style=float:left;> <?php echo utf8_encode($locsuggest);?></div>" , locationvalue: "<?php echo trim(utf8_decode($locsuggest));?>"},
...
.ac_over {
	...
	<tr>
		...
		<div style="display:none;float:right;width:220px;z-index:10000;position:absolute;margin-left:200px;margin-top:-18px;border:0px solid red;" id="textcancellation_day2"></div>
						...
						<?php } else { ?>
						<td><?php echo $cancellation_day2?></td>
						<?php } ?>                                                                                                    
					</tr>			
		...
		<tr>
						<td class="key"><label for="cancellation_fee1">
		<?php
		...
		<?php echo $req_fld?></label></td>
						<?php if ( $is_editable_field ) { ?>
						<td>
							<input type="text" name="cancellation_fee1" id="cancellation_fee1" style="width:190px;" maxlength="150" value="<?php echo fixEncoding($cancellation_fee1)?>" />
							<span class="validation-status"></span>
		<script>
		$(document).ready(function() {
		...
		<tr>
						<td class="key"><label for="cancellation_fee2">
		<?php
		...
		<?php echo $req_fld?></label></td>
						<?php if ( $is_editable_field ) { ?>
						<td>
							<input type="text" name="cancellation_fee2" id="cancellation_fee2" style="width:190px;" maxlength="150" value="<?php echo fixEncoding($cancellation_fee2)?>" />
							<span class="validation-status"></span>
		<script>
		$(document).ready(function() {
		...
		<div style="display:none;float:right;width:220px;z-index:10000;position:absolute;margin-left:200px;margin-top:-18px;border:0px solid red;" id="textcancellation_fee2"></div>
		</td>
						<?php } else { ?>
						<td><?php echo $cancellation_fee2?></td>
						<?php } ?>                                                                                                    
					</tr>
