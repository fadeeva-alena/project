	  ...
	  if ($_SESSION[WEBSITE_ALIAS]['admin_id'] ==""){
		...
		if ($rowevent['active_for_reservation'] == 1){	
			...
			if (mysql_num_rows($checkreservation) > 0){
				...
				echo "<a alt='".$reservationbutton."' title='".$reservationbutton."' style=cursor:pointer; onclick='datereserve($rowevent[eid],$size_row[id],$provider_id)'><img src='images/reservation.png' width='16px' height='16px' border='0' alt='".$makereservation."' title='".$makereservation."'/></a>";
				echo "</div>";
			}
		}
			echo "</div><div id=mailprocessing".$size_row[id]." class=mailprocessing style=float:left;></div><br style=clear:both;>";
