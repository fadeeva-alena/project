...
<?php if ($_GET['task'] == "cancelled"){
  ...
  if ($_GET['num_plan'] == 1){
	...
	$sqlcheckfirst = mysql_query("select * from t_provider_access where provider_id='".$_SESSION[WEBSITE_ALIAS]['admin_id']."' and seminar_with_reservation=1");
	...
	$sqlcheckfirst = mysql_query("select * from t_provider_access where provider_id='".$_SESSION[WEBSITE_ALIAS]['admin_id']."' and session_based=1");
	$rowcheckfirst = mysql_fetch_array($sqlcheckfirst);
	if (mysql_num_rows($sqlcheckfirst) > 0){
		mysql_query("update t_provider_access set session_based=0,session_expiration_date='$dateexpiration' where provider_id='".$_SESSION[WEBSITE_ALIAS]['admin_id']."'");
		$datetoday = date('Y-m-d H:i:s');
