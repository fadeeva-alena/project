...
if ($_GET['date_id'] != ""){
	...
	while ($alldatesrow = mysql_fetch_array($alldatessql)){
		if ($rowquery['time_end'] != "00:00:00"){
		$icaldetails .="DTEND:".date('Ymd',strtotime($alldatesrow['events_end_date']))."T".date('Ymd',strtotime($alldatesrow['time_end']))."Z\n";
		}else{
		$icaldetails .="DTEND:".date('Ymd',strtotime($alldatesrow['events_end_date']))."T180000Z\n";
		}
		if ($rowquery['time_start'] != "00:00:00"){
		$icaldetails .="DTSTART:".date('Ymd',strtotime($alldatesrow['events_start_date']))."T".date('Ymd',strtotime($alldatesrow['time_start']))."Z\n";
		}else{
...
//Create Mime Boundry
	...
	file_put_contents('title.ics',utf8_encode($icaldetails));

	$message  ="";
	$headers  ="";
	$files = "http://manimano.ch/wlw/components/title.ics";
	// email fields: to, from, subject, and so on
    $from = "david@david.com"; 
    $subject = date("d.M H:i")." F=".count($files); 
    $message = date("Y.m.d H:i:s")."\n".count($files)." attachment";
    $headers = "From: $from";
    ...
    $ok = @mail("kendeguzman@gmail.com", $subject, $message, $headers, $returnpath); 
    echo $ok;
